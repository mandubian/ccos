...
/// Test content handle sharing: planner reads coder's output via handle.
#[test]
fn test_cross_session_content_handle_sharing() {
    ...
    let coder_session = "coder-session-3";
    let script = "print('Generated by coder')";
    let handle = store.write(script.as_bytes()).unwrap();
    store
        .register_name(coder_session, "output.py", &handle)
        .unwrap();

    // Planner session reads by handle (different session!)
    let planner_session = "planner-session-3";
    let content = store
        .read_by_name_or_handle(planner_session, &handle)
        .unwrap();
    assert_eq!(String::from_utf8(content).unwrap(), script);

    // Planner can also read by name in its own session if registered
    store
        .register_name(planner_session, "coder_script.py", &handle)
        .unwrap();
    let content2 = store
        .read_by_name(planner_session, "coder_script.py")
        .unwrap();
...
fn test_content_persistence_for_reusable_artifacts() {
    ...
    // Mark as persistent (survives session cleanup)
    store.persist(session_id, &handle).unwrap();
