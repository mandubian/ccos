# CCOS-NG: Core Concepts & Architecture Vision
...
**CCOS-NG (New Generation)** is a precise plan for a modernized, expurged, and simpler iteration of CCOS. The core philosophy is to build a very thin, highly modular, and secure architecture strictly focused on autonomous reasoning agents. These agents are governed by underlying LLMs, triggered by external events (chat, network, mobile), and evolve autonomously by using and creating skills.
...
- **Multi-Agent Communities (Beyond the Single Primary)**: CCOS-NG does not restrict itself to a single "Primary Agent" talking to a single user. The architecture supports **communities** of Agents:
...
- **Persona & Constraints (Intelligence Levels)**: Each Agent has its own persona. A "Concierge" Agent acts as the front-door, powered by a top-tier model (GPT-4o, Claude 3.5 Sonnet). It doesn't need to know *how* to write Python—its intelligence is focused on **Understanding, Planning, and Routing**. It must be smart enough to:
  ...
  - *Escalation & Approvals*: If a Sub-Agent hits a strict Gateway boundary (e.g., unauthorized HTTP access), the Gateway pauses the Sub-Agent and raises an "Approval Event" to the parent Agent. The parent Agent asks the human for approval, relays the answer, and the Gateway unpauses the Sub-Agent.
...
- **Lifecycle & Spawning (External & Internal Integration)**: Sub-Agents are not inherently bound to the CCOS-NG platform's internal LLM. The Gateway can spawn and wrap any external autonomous CLI tool (like `claude-code`, `aider`, or a custom Python script) exactly as if it were a native Sub-Agent. 
...
- **The Portable Agent Manifest (Self-Contained)**: An Agent is not a database row or a hidden process. It is simply a directory (or a ZIP/tarball) containing all the information that defines it. This includes its `config.yaml` (LLM provider, max tokens, **error recovery policy**), `persona.md` (its role and rules), `state/` (working memory like `task.md`), `history/` (the Causal Chain event log), `skills/` (the text-native tools it has learned or been given), and `metrics.json` (running cost and resource usage). Because the entire agent is just a file bundle, it can be seamlessly serialized, emailed, exchanged, and respawned on completely different physical machines or CCOS-NG Gateways with zero friction.
  ...
  - *Error Recovery & Retry (Config-Driven)*: The Agent's `config.yaml` defines its recovery policy: `on_crash: restart_from_checkpoint | notify_user | abandon`. Because the Agent's `state/` directory contains the textual `task.md` checkpoint, the Gateway can automatically reboot a crashed Agent, feed it the last `task.md`, and let it continue from where it left off. For scheduled Cold Path jobs running unattended, this is critical.
...
- **The Sandbox SDK (The Bridge)**: Generated skill code running inside a Sandbox is isolated from the platform — but it still needs to interact with it. The Gateway mounts a lightweight SDK library (`ccos_sdk`) into the Sandbox. This SDK is the *only* way for sandboxed code to access platform features. It communicates with the Gateway via a local Unix socket or stdin/stdout pipe. The SDK exposes:
  - `ccos_sdk.memory.read(key)` / `ccos_sdk.memory.write(key, value)` — Read/write the Agent's working memory files.
  - `ccos_sdk.state.checkpoint(data)` — Save progress for crash recovery.
  - `ccos_sdk.secrets.get(name)` — Request a secret from the Vault (triggers an Approval Event if not pre-authorized).
  - `ccos_sdk.message.send(agent_id, payload)` — Send a message to another Agent via the Gateway message bus.
  - `ccos_sdk.message.ask(agent_id, question)` — Send a question and suspend until the answer arrives (interruptible conversation).
  - `ccos_sdk.files.upload(path)` / `ccos_sdk.files.download(url)` — Transfer files in/out of the Sandbox via the Gateway.
  - `ccos_sdk.emit(event_type, data)` — Emit a structured event to the Causal Chain and Observability stream.
- **SDK Security Boundary**: The SDK is a thin client. It does NOT execute anything locally — every call is a request to the Gateway, which enforces policies, logs the action, and decides whether to allow or block it. The generated code can call `ccos_sdk.secrets.get("GITHUB_TOKEN")`, but the Gateway decides whether that agent is authorized to access that secret.
...
Not every request needs a full LLM reasoning loop. To conserve tokens and drastically reduce latency, CCOS-NG classifies tasks into three execution paths:

- **Single Action (The Hot Path):** The user asks a direct question or commands a simple API call (e.g., "What is the BTC price?"). The Primary LLM receives the text, generates a single `ccos.skill.execute` JSON, the Gateway runs the predefined Skill, and the LLM formats the answer. Fast, simple, synchronous.
...
While CCOS-NG delegates execution to safe Sandboxes and abstracts logic from the Gateway, relying on autonomous LLMs introduces entirely new classes of vulnerabilities. The architecture must actively defend against:
...
5. **SDK Abuse (The Lateral Movement Risk):** Generated code has access to the Sandbox SDK (`ccos_sdk`), which can send messages to other agents, request secrets, and read/write memory. A maliciously crafted skill could abuse `ccos_sdk.message.send()` to spam other agents, or call `ccos_sdk.secrets.get()` attempting to enumerate secrets. *Mitigation:* Every SDK call is a Gateway request subject to the same Textual Policies. The Gateway rate-limits SDK calls per sandbox, enforces least-privilege on secret access, and caps the number of inter-agent messages a single skill execution can send.
...
3. **Define the Sandbox SDK API**: Specify the exact `ccos_sdk` Python/JS library surface, including all methods, error types, and security constraints.
