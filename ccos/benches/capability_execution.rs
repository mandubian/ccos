...
fn benchmark_security_validation(c: &mut Criterion) {
    ...
    let contexts = vec![
        ...
        (
            "controlled_1_cap",
            RuntimeContext::controlled(vec!["ccos.json.parse".to_string()]),
        ),
        (
            "controlled_10_caps",
            RuntimeContext::controlled((0..10).map(|i| format!("ccos.test.cap{}", i)).collect()),
        ),
    ];

    for (name, context) in contexts {
        group.bench_with_input(BenchmarkId::from_parameter(name), &context, |b, ctx| {
            b.iter(|| ctx.is_capability_allowed(black_box("ccos.json.parse")));
        });
    }
...
fn benchmark_value_serialization(c: &mut Criterion) {
    ...
    let test_values = vec![
        ...
        (
            "large_vector",
            Value::Vector((0..100).map(Value::Integer).collect()),
        ),
    ];
