...
fn json_capabilities_parse_and_stringify() {
    ...
    let json_string = registry
        .execute_capability_with_microvm("ccos.json.stringify", vec![value.clone()], Some(&context))
        .expect("stringify");
    ...
    let pretty = registry
        .execute_capability_with_microvm("ccos.json.stringify-pretty", vec![value], Some(&context))
