...
use rtfs::ccos::capability_marketplace::{CapabilityIsolationPolicy, ResourceConstraints};
...
async fn test_capability_marketplace_bootstrap() {
    ...
    assert!(
        count > 0,
        "Marketplace should have capabilities after bootstrap"
    );
    ...
    assert!(
        ...
        "Should be able to list capabilities"
    );

    // Test that we can check for specific capabilities
    for capability in capabilities {
        assert!(
            marketplace.has_capability(&capability.id).await,
...
async fn test_capability_marketplace_isolation_policy() {
    ...
    let allowed_result = marketplace
        .execute_capability(
            "ccos.echo",
            &Value::List(vec![Value::String("test".to_string())]),
        )
        .await;
    assert!(allowed_result.is_ok(), "Allowed capability should execute");

    // Test that denied capabilities fail
    let denied_result = marketplace
        .execute_capability(
            "ccos.math.add",
    ...
    let unknown_result = marketplace
        .execute_capability("unknown.capability", &Value::List(vec![]))
        .await;
...
async fn test_capability_marketplace_dynamic_registration() {
    ...
    marketplace
        ...
        .await
        .expect("Should register capability");

    // Verify the capability is available
    assert!(
        marketplace.has_capability("test.dynamic").await,
        "Should have dynamically registered capability"
    );

    // Test execution
    let result = marketplace
        .execute_capability("test.dynamic", &Value::List(vec![]))
        .await;
...
async fn test_capability_marketplace_audit_events() {
    ...
    assert_eq!(
        new_count,
        initial_count + 1,
        "Capability count should increase"
    );

    // Test execution (this should emit "capability_executed" audit event)
    let result = marketplace
        .execute_capability("test.audit", &Value::List(vec![]))
        .await;
    ...
    marketplace
        .remove_capability("test.audit")
        .await
        .expect("Should remove capability");

    // Verify capability count decreased
    let final_count = marketplace.capability_count().await;
    assert_eq!(
        final_count, initial_count,
        "Capability count should return to initial"
    );
...
async fn test_capability_marketplace_enhanced_isolation() {
    use rtfs::ccos::capability_marketplace::{CapabilityMarketplace, NamespacePolicy};
    ...
    assert!(
        ...
        "Namespace-allowed capability should execute"
    );
    ...
    assert!(
        ...
        "Namespace-denied capability should fail"
    );
}

#[tokio::test]
async fn test_capability_marketplace_time_constraints() {
    use rtfs::ccos::capability_marketplace::{CapabilityMarketplace, TimeConstraints};
...
async fn test_capability_marketplace_audit_integration() {
    ...
    let result = marketplace
        .execute_capability("test.audit.integration", &Value::List(vec![]))
        .await;
    assert!(result.is_ok(), "Should execute capability");

    // Remove the capability (should emit audit event)
    marketplace
        .remove_capability("test.audit.integration")
        .await
        .expect("Should remove capability");
...
async fn test_capability_marketplace_discovery_providers() {
    use rtfs::ccos::capability_marketplace::{
        discovery::StaticDiscoveryProvider, CapabilityMarketplace,
    };
    ...
    assert!(
        marketplace.has_capability("static.hello").await,
        "Should have static capability"
    );

    // Test execution of discovered capability
    let result = marketplace
        .execute_capability("static.hello", &Value::List(vec![]))
        .await;
    assert!(result.is_ok(), "Should execute static capability");

    if let Ok(Value::String(s)) = result {
        assert_eq!(
            s, "Hello from static discovery!",
            "Should return expected static result"
        );
    } else {
...
async fn test_capability_marketplace_causal_chain_integration() {
    ...
    let result = marketplace
        .execute_capability("test.causal_chain", &Value::List(vec![]))
        .await
    ...
    assert!(
        actions.len() >= 3,
        "Should have recorded capability lifecycle events"
    );

    // Check for capability lifecycle events
    let capability_events: Vec<_> = actions
        .iter()
        .filter(|action| {
            matches!(
                action.action_type,
                rtfs::ccos::types::ActionType::CapabilityRegistered
                    | rtfs::ccos::types::ActionType::CapabilityCall
            )
        })
        .collect();
    ...
    assert!(
        ...
        "Should have recorded capability events in Causal Chain"
    );

    // Verify event metadata
    for event in &capability_events {
        assert!(
            event.metadata.contains_key("capability_id"),
            "Event should have capability_id metadata"
        );
        assert!(
            event.metadata.contains_key("event_type"),
            "Event should have event_type metadata"
        );
    }

    println!(
        "✅ Causal Chain integration test passed - {} capability events recorded",
        capability_events.len()
    );
}
...
async fn test_capability_marketplace_resource_monitoring() {
    use rtfs::ccos::capability_marketplace::{
        types::ResourceMonitoringConfig, CapabilityMarketplace, ResourceConstraints,
    };
    use std::collections::HashMap;
    ...
    let mut marketplace =
        CapabilityMarketplace::with_resource_monitoring(registry, None, monitoring_config);
    ...
    assert!(
        result.is_ok(),
        "Capability execution should succeed with resource monitoring"
    );
}
...
async fn test_capability_marketplace_gpu_resource_limits() {
    use rtfs::ccos::capability_marketplace::{
        types::ResourceMonitoringConfig, CapabilityMarketplace,
    };
    use std::collections::HashMap;

    // Create constraints with GPU limits that accommodate placeholder values
    let constraints = ResourceConstraints::default().with_gpu_limits(Some(4096), Some(100.0)); // 4GB GPU memory, 100% utilization
    ...
    let result = marketplace
        .execute_capability("gpu.intensive_task", &Value::String("gpu_data".to_string()))
        .await;

    assert!(
        result.is_ok(),
        "GPU-intensive capability should execute within limits"
    );
}
...
async fn test_capability_marketplace_environmental_limits() {
    ...
    let constraints =
        ResourceConstraints::default().with_environmental_limits(Some(50.0), Some(0.2)); // 50g CO2, 0.2 kWh
    ...
    assert!(
        result.is_ok(),
        "Energy-intensive capability should execute (soft limits)"
    );
}
...
async fn test_capability_marketplace_custom_resource_limits() {
    use rtfs::ccos::capability_marketplace::{
        types::EnforcementLevel, types::ResourceMonitoringConfig, CapabilityMarketplace,
    };
    use std::collections::HashMap;
    ...
    constraints =
        constraints.with_custom_limit("api_calls", 100.0, "calls", EnforcementLevel::Hard);
...
async fn test_capability_marketplace_resource_violation_handling() {
    ...
    let constraints = ResourceConstraints::default().with_gpu_limits(Some(1), Some(1.0)); // 1MB GPU memory, 1% utilization (very low)
    ...
    let result = marketplace
        .execute_capability("test.violation", &Value::String("test".to_string()))
        .await;
    ...
    match result {
        ...
        Err(e) => {
            assert!(
                e.to_string().contains("Resource constraints violated"),
                "Error should be about resource constraints: {}",
                e
            );
        }
    }
...
async fn test_capability_marketplace_resource_monitoring_disabled() {
    ...
    let mut policy = CapabilityIsolationPolicy::default();
    policy.resource_constraints =
        Some(ResourceConstraints::default().with_gpu_limits(Some(1024), Some(50.0)));
    marketplace.set_isolation_policy(policy);
    ...
    let result = marketplace
        .execute_capability("test.no_monitoring", &Value::String("test".to_string()))
        .await;

    assert!(
        result.is_ok(),
        "Capability should execute without resource monitoring"
    );
}
...
fn observability_prometheus_render_smoke() {
    use rtfs::ccos::capability_marketplace::CapabilityMarketplace;
    use rtfs::ccos::host::RuntimeHost;
    use rtfs::runtime::metrics_exporter::render_prometheus_text;
    use rtfs::runtime::RuntimeContext;
    ...
    let causal_chain = Arc::new(Mutex::new(
        rtfs::ccos::causal_chain::CausalChain::new().expect("chain"),
    ));
