...
use rtfs::ccos::execution_context::{ConflictResolution, ContextManager, IsolationLevel};
...
fn test_context_isolation_levels() -> RuntimeResult<()> {
    ...
    manager.set(
        "shared".to_string(),
        Value::String("root_value".to_string()),
    )?;
    ...
    assert_eq!(
        manager.get("shared"),
        Some(Value::String("root_value".to_string()))
    );
...
fn test_parallel_context_execution() -> RuntimeResult<()> {
    ...
    manager.set(
        "parallel_data".to_string(),
        Value::String("context1_value".to_string()),
    )?;

    manager.switch_to(&context2_id)?;
    manager.set(
        "parallel_data".to_string(),
        Value::String("context2_value".to_string()),
    )?;

    // Verify contexts are isolated
    manager.switch_to(&context1_id)?;
    assert_eq!(
        manager.get("parallel_data"),
        Some(Value::String("context1_value".to_string()))
    );

    manager.switch_to(&context2_id)?;
    assert_eq!(
        manager.get("parallel_data"),
        Some(Value::String("context2_value".to_string()))
    );
...
fn test_merge_policy_keyword_overwrite_in_step_parallel() -> RuntimeResult<()> {
    ...
    let _expr = Expression::List(vec![
        Expression::Symbol(rtfs::ast::Symbol("step-parallel".to_string())),
        ...
        Expression::Do(rtfs::ast::DoExpr {
            expressions: vec![
                // Simulate child setting k = "child-a"
                Expression::List(vec![
                    Expression::Symbol(rtfs::ast::Symbol("set-context".to_string())),
    ...
    {
        let mut mgr = evaluator.context_manager.borrow_mut();
        let c1 = mgr.create_parallel_context(Some("b1".to_string()))?;
        mgr.switch_to(&c1)?;
        mgr.set("k".to_string(), Value::String("child-a".to_string()))?;
        mgr.merge_child_to_parent(&c1, ConflictResolution::Overwrite)?;
        mgr.switch_to("root")?;
        let c2 = mgr.create_parallel_context(Some("b2".to_string()))?;
        mgr.switch_to(&c2)?;
        mgr.set("k".to_string(), Value::String("child-b".to_string()))?;
        mgr.merge_child_to_parent(&c2, ConflictResolution::Overwrite)?;
        mgr.switch_to("root")?;
