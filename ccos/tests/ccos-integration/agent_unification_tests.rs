...
use rtfs::ccos::capability_marketplace::types::*;
use rtfs::runtime::capabilities::registry::CapabilityRegistry;
use rtfs::CapabilityMarketplace;
use std::sync::Arc;
...
async fn test_agent_unification_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let primitive_manifest = CapabilityManifest::new(
        ...
        ProviderType::Local(LocalCapability {
            handler: Arc::new(|_| Ok(rtfs::runtime::values::Value::Integer(42))),
        }),
        "1.0.0".to_string(),
    );
    ...
    marketplace
        .register_capability_manifest(primitive_manifest)
        .await?;
    marketplace
        .register_capability_manifest(agent_manifest)
        .await?;
    ...
    println!(
        "Found {} capabilities: {:?}",
        all_caps.len(),
        all_caps.iter().map(|c| &c.id).collect::<Vec<_>>()
    );
...
async fn test_capability_query_filtering() -> Result<(), Box<dyn std::error::Error>> {
    ...
    marketplace
        .register_capability_manifest(planning_agent)
        .await?;
    marketplace
        .register_capability_manifest(interactive_agent)
        .await?;

    // Test query filtering
    let planning_query = CapabilityQuery::new().with_planning(true);
    let planning_caps = marketplace
        .list_capabilities_with_query(&planning_query)
        .await;
    ...
    let interactive_caps = marketplace
        .list_capabilities_with_query(&interactive_query)
        .await;
    ...
    let primitive_caps = marketplace
        .list_capabilities_with_query(&primitive_query)
        .await;
...
async fn test_agent_metadata_helpers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let primitive = CapabilityManifest::new(
        ...
        ProviderType::Local(LocalCapability {
            handler: Arc::new(|_| Ok(rtfs::runtime::values::Value::Integer(1))),
        }),
        "1.0.0".to_string(),
    );
