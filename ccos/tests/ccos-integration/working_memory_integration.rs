...
use rtfs::ccos::event_sink::CausalChainEventSink;
use rtfs::ccos::types::{Action, ActionType, ExecutionResult};
use rtfs::ccos::wm_integration::WmIngestionSink;
use rtfs::ccos::working_memory::backend::QueryParams;
use rtfs::ccos::working_memory::backend_inmemory::InMemoryJsonlBackend;
use rtfs::ccos::working_memory::facade::WorkingMemory;
...
async fn test_end_to_end_working_memory_integration() {
    ...
    metadata1.insert(
        "content".to_string(),
        Value::Map(create_map_from_json(
            json!({"plan": "github-analysis", "status": "started"}),
        )),
    );
    ...
    let action1 = Action {
        ...
        arguments: Some(vec![Value::Map(create_map_from_json(
            json!({"repo": "mandubian/ccos", "task": "analyze"}),
        ))]),
        result: None,
        cost: Some(0.1),
    ...
    metadata2.insert(
        "content".to_string(),
        Value::Map(create_map_from_json(
            json!({"plan": "analyze-repositories", "step": 1}),
        )),
    );
    ...
    println!(
        "Found {} entries in Working Memory",
        all_actions.entries.len()
    );
    for (i, entry) in all_actions.entries.iter().enumerate() {
        println!(
            "Entry {}: ID={}, title={}, tags={:?}",
            i, entry.id, entry.title, entry.tags
        );
    }
    ...
    assert_eq!(
        all_actions.entries.len(),
        2,
        "Expected 2 actions to be ingested"
    );

    // Check that entries contain expected action IDs in their working memory IDs
    let has_action_001 = all_actions
        .entries
        .iter()
        .any(|e| e.id.contains("action-001"));
    let has_action_002 = all_actions
        .entries
        .iter()
        .any(|e| e.id.contains("action-002"));
...
async fn test_working_memory_query_apis_for_arbiter_context_horizon() {
    ...
    for i in 0..15 {
        ...
        metadata.insert(
            "content".to_string(),
            Value::Map(create_map_from_json(content.clone())),
        );

        let action = Action {
            action_id: format!("action-{:03}", i + 1),
            parent_action_id: if i > 0 {
                Some(format!("action-{:03}", i))
            } else {
                None
            },
    ...
    let all_actions = wm.query(&all_query).unwrap();
    assert!(
        all_actions.entries.len() >= 15,
        "Should have 15 actions ingested"
    );
    ...
    println!(
        "Now in ms: {}, fifteen_min_ago in ms: {}",
        now,
        now - 900000
    );
    ...
    println!(
        "Found {} recent actions out of {} total",
        recent_actions.entries.len(),
        all_actions.entries.len()
    );
    assert!(
        recent_actions.entries.len() >= 7,
        "Recent actions should be available"
    );
    ...
    assert!(
        capability_calls.entries.len() >= 2,
        "Should find capability call actions"
    );

    let plan_query = QueryParams::with_tags(["planstepstarted"]).with_limit(Some(10));
    let plan_actions = wm.query(&plan_query).unwrap();
    assert!(
        plan_actions.entries.len() >= 2,
        "Should find plan step actions"
    );
...
async fn test_working_memory_bridge_integration() {
    ...
    assert!(
        entry.id.contains("test-action-001"),
        "Entry ID should contain action ID"
    );
