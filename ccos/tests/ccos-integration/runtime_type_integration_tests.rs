...
use rtfs::ccos::types::ExecutionResult;
use rtfs::parser;
...
use rtfs::runtime::type_validator::{TypeCheckingConfig, ValidationLevel};
use rtfs::runtime::{Evaluator, Value};
...
mod hybrid_runtime_integration_tests {
    ...
    fn test_arithmetic_with_optimization() {
        ...
        assert!(
            result.is_ok(),
            "Expected successful evaluation, got: {:?}",
            result
        );
        match result.unwrap() {
            rtfs::runtime::execution_outcome::ExecutionOutcome::Complete(value) => {
    ...
    fn test_function_definition_with_validation() {
        ...
        let def_parsed =
            parser::parse_expression(def_input).expect("Failed to parse function definition");
        ...
        assert!(
            def_result.is_ok(),
            "Expected successful function definition, got: {:?}",
            def_result
        );
        ...
        let call_parsed =
            parser::parse_expression(call_input).expect("Failed to parse function call");
        ...
        assert!(
            call_result.is_ok(),
            "Expected successful function call, got: {:?}",
            call_result
        );
    ...
    fn test_map_operations() {
        ...
        assert!(result.is_ok());
        match result.unwrap() {
            rtfs::runtime::execution_outcome::ExecutionOutcome::Complete(Value::Map(
                map,
            )) => {
                assert_eq!(map.len(), 2);
            }
    ...
    fn test_configuration_effects() {
        ...
        assert!(
            optimized_evaluator
                .get_type_checking_config()
                .skip_compile_time_verified
        );
    ...
    fn test_complex_expression_evaluation() {
        ...
        assert!(
            result.is_ok(),
            "Expected successful complex evaluation, got: {:?}",
            result
        );
        match result.unwrap() {
            rtfs::runtime::execution_outcome::ExecutionOutcome::Complete(value) => {
    ...
    fn test_performance_characteristics() {
        ...
        match (result1.unwrap(), result2.unwrap()) {
            (
                rtfs::runtime::execution_outcome::ExecutionOutcome::Complete(value1),
                rtfs::runtime::execution_outcome::ExecutionOutcome::Complete(value2),
            ) => {
