...
fn test_pure_context() {
    ...
    let result = evaluator.eval_expr(&pure_expr, &mut evaluator.env.clone());
...
fn test_controlled_context() {
    ...
    assert!(evaluator
        .security_context
        .is_capability_allowed("ccos.echo"));
    assert!(!evaluator
        .security_context
        .is_capability_allowed("ccos.unauthorized"));
...
fn test_full_context() {
    ...
    assert!(evaluator
        .security_context
        .is_capability_allowed("ccos.math.add"));
    assert!(evaluator
        .security_context
        .is_capability_allowed("any.capability"));
