use rtfs::ast::{Keyword, MapTypeEntry, PrimitiveType, TypeExpr, TypePredicate};
...
async fn test_map_type_validation() {
    ...
    let handler = Arc::new(|inputs: &Value| -> RuntimeResult<Value> {
        if let Value::Map(map) = inputs {
            let name = map
                .get(&rtfs::ast::MapKey::Keyword(Keyword::new("name")))
                .and_then(|v| match v {
                    Value::String(s) => Some(s.clone()),
                    _ => None,
                })
            ...
            let count = map
                .get(&rtfs::ast::MapKey::Keyword(Keyword::new("count")))
                .and_then(|v| match v {
    ...
    input_map.insert(
        rtfs::ast::MapKey::Keyword(Keyword::new("name")),
        Value::String("apple".to_string()),
    );
    input_map.insert(
        rtfs::ast::MapKey::Keyword(Keyword::new("count")),
        Value::Integer(5),
    );
