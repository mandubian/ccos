...
use ccos::streaming::{McpStreamingProvider, StreamStatus, StreamingCapability};
use std::sync::Arc;
...
async fn test_phase3_processor_invocation() {
    ...
    let invoker = Arc::new(
        move |name: &str,
            ...
            if seq == 3 {
                ...
                out.insert(
                    MapKey::Keyword(Keyword("final-count".into())),
                    Value::Integer(new_count),
                );
    ...
    assert_eq!(
        ...
        "Expected Completed status from processor directive"
    );
}
...
async fn test_phase3_invalid_return_shape_errors() {
    ...
    let invoker = Arc::new(
        |name: &str, _state: &Value, _chunk: &Value, _metadata: &Value| -> RuntimeResult<Value> {
            if name == "bad-processor" {
                Ok(Value::Integer(42))
            } else {
                Err(RuntimeError::Generic("missing".into()))
            }
        },
    );
    let provider = McpStreamingProvider::new_with_invoker("http://localhost/mock".into(), invoker);
    ...
    param_map.insert(
        MapKey::Keyword(Keyword("processor".into())),
        Value::String("bad-processor".into()),
    );
    param_map.insert(
        MapKey::Keyword(Keyword("initial-state".into())),
        Value::Map(std::collections::HashMap::new()),
    ...
    if let Some(StreamStatus::Error(msg)) = provider.get_status(&stream_id) {
        assert!(msg.contains("invalid shape"), "Unexpected msg: {}", msg);
    } else {
