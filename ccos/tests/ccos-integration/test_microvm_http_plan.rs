...
fn test_microvm_http_fetch() {
    ...
    let runtime_context = RuntimeContext::controlled(vec!["ccos.network.http-fetch".to_string()]);
    ...
    match result {
        ...
        Ok(Value::Map(map)) => {
            ...
            let status = map.get(&rtfs::ast::MapKey::String("status".to_string()));
            let body = map.get(&rtfs::ast::MapKey::String("body".to_string()));
