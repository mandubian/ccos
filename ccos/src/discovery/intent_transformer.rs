...
use crate::types::{GenerationContext, Intent, IntentStatus};
...
impl IntentTransformer {
    ...
    pub fn need_to_intent(
        ...
        metadata.insert(
            ...
            Value::String(need.capability_class.clone()),
        );
        metadata.insert(
            "synthesis_source".to_string(),
            Value::String("recursive_discovery".to_string()),
        );
...
mod tests {
    ...
    fn test_need_to_intent() {
        ...
        assert_eq!(
            intent.name,
            Some("Synthesize restaurant.reservation.book".to_string())
        );
