...
impl IntrospectionCache {
    ...
    pub fn new(cache_dir: impl AsRef<Path>) -> RuntimeResult<Self> {
        ...
        fs::create_dir_all(&cache_dir).map_err(|e| {
            RuntimeError::Generic(format!("Failed to create cache directory: {}", e))
        })?;
    ...
    pub fn get_mcp(&self, url: &str) -> RuntimeResult<Option<MCPIntrospectionResult>> {
        ...
        let entry: CacheEntry<MCPIntrospectionResult> =
            serde_json::from_str(&content).map_err(|e| {
                RuntimeError::Generic(format!("Failed to deserialize cache entry: {}", e))
            })?;
    ...
    pub fn put_mcp(&self, url: &str, data: &MCPIntrospectionResult) -> RuntimeResult<()> {
        ...
        let json = serde_json::to_string_pretty(&entry).map_err(|e| {
            RuntimeError::Generic(format!("Failed to serialize cache entry: {}", e))
        })?;
    ...
    pub fn get_openapi(&self, url: &str) -> RuntimeResult<Option<APIIntrospectionResult>> {
        ...
        let entry: CacheEntry<APIIntrospectionResult> =
            serde_json::from_str(&content).map_err(|e| {
                RuntimeError::Generic(format!("Failed to deserialize cache entry: {}", e))
            })?;
        ...
        eprintln!(
            "  📦 Using cached OpenAPI introspection result for: {}",
            url
        );
        Ok(Some(entry.data))
    }
            ...
            for entry in fs::read_dir(&self.cache_dir).map_err(|e| {
                ...
                if path.is_file() {
                    fs::remove_file(&path).map_err(|e| {
                        RuntimeError::Generic(format!("Failed to remove cache file: {}", e))
                    })?;
                }
            }
        }
            ...
            if path.is_file() {
                ...
                if let Ok(content) = fs::read_to_string(&path) {
                    ...
                    if let Ok(cache_entry) =
                        serde_json::from_str::<CacheEntry<MCPIntrospectionResult>>(&content)
                    {
                    ...
                    else if let Ok(cache_entry) =
                        serde_json::from_str::<CacheEntry<APIIntrospectionResult>>(&content)
                    {
                        if self.is_expired(&cache_entry) {
                            stats.expired_entries += 1;
                        }
