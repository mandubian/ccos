...
use crate::planner::modular_planner::types::{IntentType, SubIntent, ApiAction};
...
mod tests {
            ...
            self.capabilities
                ...
                .filter(|c| {
                    ...
                    query_words.iter().any(|word| {
                        word.len() > 3 && cap_text.contains(*word)
                    })
                })
                .take(limit)
                .cloned()
    ...
    async fn test_semantic_resolution() {
        ...
        match result {
            ResolvedCapability::Local { capability_id, arguments, .. } => {
