...
use crate::skills::loader::{parse_skill_markdown, SkillFormat};
...
impl SkillMapper {
    ...
    /// Load a skill from raw content and register it.
    pub async fn load_from_content(
        &mut self,
        content: &str,
        source_hint: &str,
    ) -> Result<crate::skills::LoadedSkillInfo, SkillError> {
        let hint = source_hint.to_lowercase();
        let format = if hint.ends_with(".md") || hint.ends_with(".markdown") {
            SkillFormat::Markdown
        } else if hint.ends_with(".yaml") || hint.ends_with(".yml") {
            SkillFormat::Yaml
        } else {
            SkillFormat::Unknown
        };
        ...
        let skill = match format {
            SkillFormat::Markdown => parse_skill_markdown(content).map_err(|e| {
                SkillError::Execution(format!("Failed to parse markdown skill: {}", e))
            })?,
            SkillFormat::Yaml => crate::skills::parse_skill_yaml(content)
                .map_err(|e| SkillError::Execution(format!("Failed to parse yaml skill: {}", e)))?,
            _ => parse_skill_markdown(content)
                .or_else(|_| crate::skills::parse_skill_yaml(content))
                .map_err(|e| {
                    SkillError::Execution(format!("Failed to parse skill content: {}", e))
                })?,
        };

        let mut loaded = crate::skills::LoadedSkillInfo {
            capabilities_to_register: skill.capabilities.clone(),
            requires_approval: skill.approval.required || !skill.secrets.is_empty(),
            skill: skill.clone(),
            source_url: source_hint.to_string(),
            format,
        };

        let registered_cap_ids = self
            .register_skill_capabilities(&skill, Some(source_hint))
            .await?;
    ...
    pub async fn register_skill_capabilities(
        ...
        for op in &skill.operations {
            let normalized_op = op.name.to_lowercase().replace(' ', "-").replace('_', "-");
            let cap_id = format!("{}.{}", skill.id, normalized_op);
                                ...
                                && second_mapped.capability_id == "ccos.json.parse"
                            {
                                ...
                                let handler = Arc::new(move |inputs: &Value| {
                                    ...
                                    Box::pin(async move {
                                        ...
                                        let final_params = first_mapped.params.clone();

                                        let rtfs_params = merge_rtfs_inputs_and_secrets(
                                            final_params,
                                            inputs,
                                            &skill_secrets,
                                        )?;
...
/// Helper function to merge RTFS inputs and Secrets into the final params Map
fn merge_rtfs_inputs_and_secrets(
    mut final_params: HashMap<String, serde_json::Value>,
    inputs: &Value,
    skill_secrets: &[String],
) -> RuntimeResult<Value> {
        ...
        serde_json::Value::Object(obj) => {
            for (k, v) in obj {
                ...
                if k == "body" && (v.is_object() || v.is_array()) {
                    final_params.insert(k, serde_json::Value::String(v.to_string()));
                } else {
    ...
    let rtfs_params_json =
        serde_json::Value::Object(final_params.into_iter().collect::<serde_json::Map<_, _>>());
    json_to_rtfs_value(&rtfs_params_json)
}
