...
impl DummyArbiter {
    ...
    pub fn new(config: CognitiveEngineConfig, intent_graph: Arc<Mutex<IntentGraph>>) -> Self {
        Self {
            config,
            intent_graph,
        }
...
#[async_trait(?Send)]
impl CognitiveEngine for DummyArbiter {
  ...
  (edge :DependsOn \"announce_results\" \"analyze_data\"))"#,
        ...
        let mut g = self
            ...
            .lock()
            .map_err(|_| RuntimeError::Generic("Failed to lock IntentGraph".to_string()))?;
        match crate::rtfs_bridge::graph_interpreter::build_graph_from_rtfs(&rtfs, &mut g) {
