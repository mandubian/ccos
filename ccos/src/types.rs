...
pub use crate::capability_marketplace::CapabilityMarketplace;
...
impl StorableIntent {
    ...
    pub fn to_runtime_intent(&self, ccos: &CCOS) -> Result<RuntimeIntent, RuntimeError> {
        let rtfs_runtime_arc = ccos.get_rtfs_runtime();
        let mut rtfs_runtime = rtfs_runtime_arc.lock().unwrap();
...
impl RuntimeIntent {
    ...
    pub fn to_storable_intent(&self, ccos: &CCOS) -> Result<StorableIntent, RuntimeError> {
        let rtfs_runtime_arc = ccos.get_rtfs_runtime();
        let rtfs_runtime = rtfs_runtime_arc.lock().unwrap();
    ...
    pub fn evaluate_success_criteria(&self, _context: &CCOSContext) -> Result<Value, RuntimeError> {
        if let Some(_criteria) = &self.success_criteria {
            // TODO: Implement CCOS-level evaluation after RTFS/CCOS separation
            // Criteria evaluation needs to be implemented at CCOS orchestration level
            Ok(Value::Boolean(true)) // Temporary: assume success
        } else {
    ...
    pub fn evaluate_constraint(
        &self,
        name: &str,
        _context: &CCOSContext,
    ) -> Result<Value, RuntimeError> {
        if let Some(_constraint) = self.constraints.get(name) {
            // TODO: Implement CCOS-level constraint evaluation after RTFS/CCOS separation
            // Constraint evaluation needs to be implemented at CCOS orchestration level
            Ok(Value::Boolean(true)) // Temporary: assume constraint satisfied
        } else {
    ...
    pub fn evaluate_preference(
        ...
        if let Some(_preference) = self.preferences.get(name) {
            // TODO: Implement CCOS-level preference evaluation after RTFS/CCOS separation
            // Preference evaluation needs to be implemented at CCOS orchestration level
            Ok(Value::Nil) // Temporary: return nil for preferences
        } else {
            Err(RuntimeError::Generic(format!(
...
impl CCOSContext<'_> {
    ...
    pub fn intent_graph(&self) -> Arc<Mutex<super::intent_graph::IntentGraph>> {
        self.ccos.get_intent_graph()
    }

    pub fn causal_chain(&self) -> Arc<Mutex<super::causal_chain::CausalChain>> {
        self.ccos.get_causal_chain()
    }

    pub fn capability_marketplace(
        &self,
    ) -> std::sync::Arc<crate::capability_marketplace::CapabilityMarketplace> {
        self.ccos.get_capability_marketplace()
    }
}
