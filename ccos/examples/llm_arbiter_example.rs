...
use ccos::arbiter::llm_provider::{LlmProvider, LlmProviderConfig, StubLlmProvider};
use ccos::arbiter::{ArbiterConfig, LlmProviderType};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let llm_config = LlmProviderConfig {
        ...
        retry_config: ccos::arbiter::arbiter_config::RetryConfig::default(),
    };
    ...
    let test_intent = StorableIntent {
        ...
        parent_intent: None,
        child_intents: vec![],
        triggered_by: ccos::types::TriggerSource::HumanRequest,
        generation_context: ccos::types::GenerationContext {
    ...
    if let ccos::types::PlanBody::Rtfs(rtfs_code) = &plan.body {
        println!("      RTFS Code:");
        println!("      {}", rtfs_code);
    ...
    let arbiter_config = ArbiterConfig {
        engine_type: ccos::arbiter::arbiter_config::ArbiterEngineType::Llm,
        llm_config: Some(ccos::arbiter::arbiter_config::LlmConfig {
            provider_type: LlmProviderType::Stub,
            model: "stub-model-v1".to_string(),
            api_key: None,
            base_url: None,
