...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    auth_headers.insert("Authorization".to_string(), format!("Bearer {}", github_token));
    ...
    match introspector.introspect_mcp_server_with_auth(
        &github_mcp_url,
        "github",
        Some(auth_headers),
    ).await {
        Ok(introspection) => {
            ...
            match introspector.create_capabilities_from_mcp(&introspection) {
                Ok(capabilities) => {
                    ...
                    let mut matches_with_scores: Vec<(ccos::capability_marketplace::types::CapabilityManifest, f64, f64, String)> = Vec::new();
                    let threshold = 0.5;
                    ...
                    if embedding_service.is_some() {
                        ...
                        println!("  → Using keyword-based matching (embedding service not configured)\n");
                    ...
                    for manifest in &capabilities {
                        ...
                        let match_type = if desc_score > name_score { "description".to_string() } else { "name".to_string() };
                    ...
                    for (idx, (manifest, desc_score, name_score, match_type)) in matches_with_scores.iter().enumerate() {
                        let best_score = desc_score.max(*name_score);
                        let rank_marker = if idx == 0 && best_score >= threshold { "🥇" } else if idx == 1 && best_score >= threshold { "🥈" } else if idx == 2 && best_score >= threshold { "🥉" } else { "  " };
                        
                        println!("{} [{}/{}] {}", rank_marker, idx + 1, matches_with_scores.len(), manifest.id);
                        println!("     Description: {}", manifest.description);
                        println!("     • Description match: {:.3} (matches rationale)", desc_score);
                        println!("     • Name match: {:.3} (matches capability class)", name_score);
                    ...
                    let best_match = matches_with_scores.iter()
                    ...
                    if let Some((manifest, desc_score, name_score, match_type)) = best_match {
                        ...
                        println!("      • Description match: {:.3} (primary - matches rationale)", desc_score);
                        println!("      • Name match: {:.3} (secondary - matches class)", name_score);
                        println!("      • Best score: {:.3} ({}-based)", best_score, match_type);
                        println!();
                        println!("   💡 Why this was chosen:");
                        if *match_type == "description" {
                            println!("      ✓ Description-based matching prioritized (score: {:.3})", desc_score);
                            println!("      ✓ Rationale '{}' semantically matches", need.rationale);
                            println!("      ✓ Capability description: '{}'", manifest.description);
                        } else {
                            println!("      ✓ Name-based matching used (description score: {:.3})", desc_score);
                            println!("      ✓ Capability class '{}' matches ID '{}'", need.capability_class, manifest.id);
                        }
                        println!("      ✓ Score {:.3} exceeds threshold {:.1}", best_score, threshold);
                    } else {
                        println!("{}", "═".repeat(80));
                        ...
                        if let Some((manifest, desc_score, name_score, _)) = matches_with_scores.first() {
                            let best_score = desc_score.max(*name_score);
                            println!("      • {}", manifest.id);
                            println!("      • Description: {}", manifest.description);
                            println!("      • Best score: {:.3} (need {:.1} to match)", best_score, threshold);
                            println!();
                            println!("   💡 Suggestions:");
                            println!("      • Lower threshold to {:.1} to include this capability", best_score + 0.1);
