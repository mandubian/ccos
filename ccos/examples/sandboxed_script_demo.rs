use ccos::capability_marketplace::executors::{SandboxedExecutor, CapabilityExecutor};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    input_map.insert(MapKey::String("name".to_string()), Value::String("CCOS User".to_string()));
    input_map.insert(MapKey::String("task".to_string()), Value::String("demonstrate generic sandboxing".to_string()));
    ...
    match executor.execute(&python_process, &input).await {
    ...
    match executor.execute(&python_fc, &input).await {
        Ok(result) => println!("✅ Result: {:?}\n", result),
        Err(e) => println!("❌ Error: {:?}\n", e),
    }
    ...
    large_map.insert(MapKey::String("large_data".to_string()), Value::String(large_string));
    ...
    match executor.execute(&python_large, &large_input).await {
        Ok(result) => println!("✅ Result: {:?}\n", result),
        Err(e) => println!("❌ Error: {:?}\n", e),
    }
