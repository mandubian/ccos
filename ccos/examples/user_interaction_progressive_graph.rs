...
use rtfs::config::{auto_select_model, expand_profiles};
use rtfs::runtime::security::{RuntimeContext, SecurityLevel};
    ...
    for turn in 0..max_turns {
        ...
        if emit_schema {
            if let Ok(chain_lock) = ccos.get_causal_chain().lock() {
                use ccos::synthesis::preference_schema::extract_with_metrics;
            ...
            if let Some(block) = extract_capability_block(&clarified_raw) {
                ...
                match plan_result {
                    Ok(plan) => {
                        ...
                        if let ccos::types::PlanBody::Rtfs(plan_body) = plan.body {
...
async fn synthesize_agent_from_interactions(
    ...
    let rtfs_plan = match plan.body {
        ccos::types::PlanBody::Rtfs(plan_code) => plan_code,
    ...
    let agent_descriptor = ccos::agent::registry::AgentDescriptor {
        ...
        trust_tier: ccos::agent::registry::TrustTier::T1Trusted,
        cost: ccos::agent::registry::CostModel::default(),
        latency: ccos::agent::registry::LatencyStats::default(),
        success: ccos::agent::registry::SuccessStats::default(),
...
fn parse_and_validate_capability(spec: &str) -> Result<(), String> {
    ...
    match rtfs::parser::parse_with_enhanced_errors(&src, None) {
        Ok(items) => {
            // Ensure at least one TopLevel::Capability is present
            for tl in items.iter() {
                match tl {
                    rtfs::ast::TopLevel::Capability(_) => return Ok(()),
                    _ => {}
                }
            }
...
impl ResponseHandler {
        ...
        if let Ok(chain) = ccos.get_causal_chain().lock() {
            ...
            for action in actions.iter().rev() {
                if action.plan_id == plan_id
                    && action.action_type == ccos::types::ActionType::PlanPaused
                {
    ...
    fn extract_question_from_action(
        ...
        if let Some(args) = &action.arguments {
            if args.len() >= 2 {
                if let rtfs::runtime::values::Value::String(prompt) = &args[1] {
        ...
        match &result.value {
            ...
            rtfs::runtime::values::Value::Map(map) => {
                ...
                for (key, value) in map {
                    if let rtfs::runtime::values::Value::String(val_str) = value {
...
fn extract_question_prompt_from_action(
    ...
    if let Some(args) = &action.arguments {
        if args.len() >= 2 {
            match &args[1] {
                rtfs::runtime::values::Value::String(prompt) => {
                ...
                rtfs::runtime::values::Value::Map(map) => {
                    ...
                    for (_k, v) in map.iter() {
                        if let rtfs::runtime::values::Value::String(s) = v {
                            return Some(s.clone());
                        }
                    }
                }
...
fn find_latest_plan_checkpoint(ccos: &Arc<CCOS>, plan_id: &str) -> Option<String> {
    ...
    let latest_checkpoint = plan_actions
        ...
        .filter_map(|action| {
            ...
            let ccos::types::Action {
                action_type,
                metadata,
                ..
            } = action;
            if *action_type == ccos::types::ActionType::PlanPaused {
