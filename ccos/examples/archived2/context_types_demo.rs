...
fn main() -> RuntimeResult<()> {
    ...
    println!(
        "   Max Memory: {:?} bytes",
        runtime_context.max_memory_usage
    );
    println!(
        "   Max Execution Time: {:?} ms",
        runtime_context.max_execution_time
    );
    ...
    println!(
        "   Environment created with {} capabilities",
        ccos_env.list_capabilities().len()
    );
    println!("   Security Level: {:?}", ccos_env.config().security_level);
    println!(
        "   Enabled Categories: {:?}",
        ccos_env.config().enabled_categories
    );
    println!();
    ...
    exec_context.set(
        ...
        Value::String("session456".to_string()),
    )?;
    exec_context.set(
    ...
    println!(
        "   Root Context ID: {:?}",
        exec_context.current_context_id()
    );
    ...
    println!(
        "     - processing_result: {:?}",
        exec_context.get("processing_result")
    );
