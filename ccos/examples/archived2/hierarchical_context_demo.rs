...
use ccos::execution_context::{ContextManager, IsolationLevel};
...
fn main() -> RuntimeResult<()> {
    ...
    println!(
        "✓ Initialized root context: {}",
        context_manager.current_context_id().unwrap()
    );

    // Set some root-level configuration
    context_manager.set(
        "workflow_name".to_string(),
        Value::String("data-processing-pipeline".to_string()),
    )?;
    ...
    println!(
        "  - workflow_name: {}",
        context_manager.get("workflow_name").unwrap()
    );
    println!(
        "  - max_retries: {}",
        context_manager.get("max_retries").unwrap()
    );
    println!(
        "  - timeout_seconds: {}",
        context_manager.get("timeout_seconds").unwrap()
    );

    // Demonstrate step execution with inheritance
    println!("\n--- Step 1: Data Validation (Inherit) ---");
    let validation_step_id =
        context_manager.enter_step("data-validation", IsolationLevel::Inherit)?;
    ...
    println!(
        "  - validation_rules: {}",
        context_manager.get("validation_rules").unwrap()
    );
    println!(
        "  - records_processed: {}",
        context_manager.get("records_processed").unwrap()
    );

    // Simulate processing some records
    context_manager.set("records_processed".to_string(), Value::Integer(1500))?;
    println!(
        "✓ Updated records_processed: {}",
        context_manager.get("records_processed").unwrap()
    );
    ...
    if let Some(result) = validation_result {
        println!(
            "  - Step result: {} records validated",
            result.data.get("records_processed").unwrap()
        );
    }

    // Demonstrate isolated step execution
    println!("\n--- Step 2: Data Transformation (Isolated) ---");
    let transform_step_id =
        context_manager.enter_step("data-transformation", IsolationLevel::Isolated)?;
    ...
    context_manager.set(
        ...
        Value::String("normalize".to_string()),
    )?;
    ...
    println!(
        "  - transformation_type: {}",
        context_manager.get("transformation_type").unwrap()
    );

    // Simulate transformation
    context_manager.set("records_transformed".to_string(), Value::Integer(1200))?;
    println!(
        "✓ Updated records_transformed: {}",
        context_manager.get("records_transformed").unwrap()
    );
    ...
    if let Some(result) = transform_result {
        println!(
            "  - Step result: {} records transformed",
            result.data.get("records_transformed").unwrap()
        );
    }
    ...
    println!(
        ...
        context_manager
            .get("workflow_name")
            .unwrap_or(Value::String("NOT ACCESSIBLE".to_string()))
    );
    ...
    println!(
        "  - error_count: {}",
        context_manager.get("error_count").unwrap()
    );
    println!(
        "  - error_severity: {}",
        context_manager.get("error_severity").unwrap()
    );
    ...
    let parallel1_id =
        context_manager.create_parallel_context(Some("parallel-processor-1".to_string()))?;
    let parallel2_id =
        context_manager.create_parallel_context(Some("parallel-processor-2".to_string()))?;
    ...
    println!(
        "  - processor_id: {}",
        context_manager.get("processor_id").unwrap()
    );
    println!(
        "  - items_processed: {}",
        context_manager.get("items_processed").unwrap()
    );
    ...
    println!(
        "  - workflow_name: {}",
        restored_manager.get("workflow_name").unwrap()
    );
    println!(
        "  - max_retries: {}",
        restored_manager.get("max_retries").unwrap()
    );
    println!(
        "  - timeout_seconds: {}",
        restored_manager.get("timeout_seconds").unwrap()
    );
