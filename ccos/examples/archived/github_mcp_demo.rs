...
use rtfs::runtime::capabilities::provider::{
    CapabilityProvider, ExecutionContext, ProviderConfig,
};
use rtfs::runtime::capabilities::providers::github_mcp::GitHubMCPCapability;
use rtfs::runtime::Value as RuntimeValue;
    ...
    match args.action.as_str() {
        "list" => {
            ...
            let inputs = RuntimeValue::Map({
                ...
                map.insert(
                    MapKey::String("tool".to_string()),
                    RuntimeValue::String("list_issues".to_string()),
                );
                map.insert(
                    MapKey::String("arguments".to_string()),
                    RuntimeValue::Map({
                        let mut args_map = HashMap::new();
                        args_map.insert(
                            MapKey::String("owner".to_string()),
                            RuntimeValue::String(args.owner),
                        );
                        args_map.insert(
                            MapKey::String("repo".to_string()),
                            RuntimeValue::String(args.repo),
                        );
                        args_map.insert(
                            MapKey::String("state".to_string()),
                            RuntimeValue::String("open".to_string()),
                        );
                        args_map.insert(
                            MapKey::String("per_page".to_string()),
        ...
        "create" => {
            ...
            let body = args
                .body
                .unwrap_or_else(|| "Created via GitHub MCP Demo".to_string());
            ...
            let inputs = RuntimeValue::Map({
                ...
                map.insert(
                    MapKey::String("tool".to_string()),
                    RuntimeValue::String("create_issue".to_string()),
                );
                map.insert(
                    MapKey::String("arguments".to_string()),
                    RuntimeValue::Map({
                        ...
                        args_map.insert(
                            MapKey::String("labels".to_string()),
        ...
        "close" => {
            let issue_number = args
                .issue_number
                .expect("--issue-number required for close action");

            println!(
                "\n🔒 Closing issue #{} in {}/{}...",
                issue_number, args.owner, args.repo
            );
            ...
            let inputs = RuntimeValue::Map({
                ...
                map.insert(
                    MapKey::String("tool".to_string()),
                    RuntimeValue::String("close_issue".to_string()),
                );
                map.insert(
                    MapKey::String("arguments".to_string()),
                    RuntimeValue::Map({
                        ...
                        args_map.insert(
                            MapKey::String("issue_number".to_string()),
                            RuntimeValue::Integer(issue_number as i64),
                        );
                        args_map.insert(
                            MapKey::String("comment".to_string()),
                            RuntimeValue::String("Closed via GitHub MCP Demo".to_string()),
                        );
                        args_map
                    }),
                );
        ...
        _ => {
            println!(
                "❌ Unknown action: {}. Use: list, create, or close",
                args.action
            );
            println!("\nUsage examples:");
            println!("  # List issues");
