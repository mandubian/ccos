...
use ccos::discovery::embedding_service::{EmbeddingService, EmbeddingProvider};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let embedding1 = match service.embed(text1).await {
        Ok(embedding) => {
            println!("  ✓ Success! Generated embedding of {} dimensions", embedding.len());
    ...
    println!();
    println!("💡 Next steps:");
    println!("  • Use this in capability matching: export LOCAL_EMBEDDING_URL=\"{}\"", base_url);
