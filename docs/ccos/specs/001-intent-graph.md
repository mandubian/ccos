...
**Version:** 1.0  
**Date:** 2025-09-20  
**Related:** [000: Architecture](./000-ccos-architecture-new.md), [006: Cognitive Engine](./006-cognitive-engine-and-cognitive-control.md)  
...
Why critical? Agents need context. The graph provides it: Query past intents to build relevant payloads for the Cognitive Engine, ensuring plans align with long-term objectives. Immutability: Updates create new versions, preserving history.
...
{:id :intent-456
 ...
 :provenance {:from-action :action-789 :cognitive engine-version \"gpt-4o\"}}
```
...
- **Creation**: Cognitive Engine adds root intent from user input.
...
Intents are inputs to plan generation: Cognitive Engine queries graph, builds context payload (summarized via `:context.horizon` yield), generates pure RTFS plan referencing intent ID.

**Reentrant Process Example**:
1. User: \"Analyze reviews.\" → Cognitive Engine creates Intent-123, queries graph (empty → root).
...
Reentrancy shines: If paused (e.g., budget check), resume by loading graph state + partial chain, re-invoking Cognitive Engine with updated intent.
