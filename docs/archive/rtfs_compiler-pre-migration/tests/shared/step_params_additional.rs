...
fn step_params_evaluation_error_prevents_body() -> Result<(), String> {
    ...
    let mut evaluator =
        Evaluator::with_environment(module_registry, stdlib_env, security_context, host);

    evaluator
        .context_manager
        .borrow_mut()
        .initialize(Some("root".to_string()));
    evaluator.host.set_execution_context(
        "plan1".to_string(),
        vec!["intent1".to_string()],
        "root".to_string(),
    );

    use rtfs_compiler::ast::{DoExpr, Expression, Keyword, Literal, MapKey, Symbol, TopLevel};
    let mut params_map: std::collections::HashMap<MapKey, Expression> =
        std::collections::HashMap::new();
    // undefined symbol should cause eval error
    params_map.insert(
        MapKey::String("a".to_string()),
        Expression::Symbol(Symbol("no_such_var".to_string())),
    );
    ...
    let do_expr = Expression::Do(DoExpr {
        expressions: vec![step_expr],
    });
    ...
    assert!(
        err.contains("UndefinedSymbol") || err.contains("ParamBinding") || err.contains("Generic"),
        "unexpected error: {}",
        err
    );
    Ok(())
...
fn step_params_non_string_key_rejected() -> Result<(), String> {
    ...
    params_map.insert(
        MapKey::Keyword(Keyword("k".to_string())),
    ...
    let err = format!("{:?}", res.err().unwrap());
    assert!(
        err.contains("InvalidArguments") || err.contains("string keys"),
        "unexpected error: {}",
        err
    );
    Ok(())
...
fn step_params_shadowing_for_nested_steps() -> Result<(), String> {
    ...
    let mut outer_params: std::collections::HashMap<MapKey, Expression> =
        std::collections::HashMap::new();
    outer_params.insert(
    ...
    let mut inner_params: std::collections::HashMap<MapKey, Expression> =
        std::collections::HashMap::new();
    inner_params.insert(
        MapKey::String("x".to_string()),
    ...
    let do_expr = Expression::Do(DoExpr {
        expressions: vec![outer_step],
    });
    let program = vec![TopLevel::Expression(do_expr)];

    let result = evaluator
        .eval_toplevel(&program)
    ...
    match result {
        rtfs_compiler::runtime::execution_outcome::ExecutionOutcome::Complete(
            ...
            assert_eq!(
                s, "outer",
                "outer %params should be visible after inner step returned"
            );
            Ok(())
        }
        rtfs_compiler::runtime::execution_outcome::ExecutionOutcome::Complete(other) => {
            Err(format!("unexpected result: {:?}", other))
        }
        rtfs_compiler::runtime::execution_outcome::ExecutionOutcome::RequiresHost(_) => {
            Err("Unexpected host call in pure test".to_string())
        }
    }
}
