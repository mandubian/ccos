...
fn main() -> RuntimeResult<()> {
    ...
    println!(
        ...
        settings.default_config.memory_limit_mb
    );
    println!(
        "  Default CPU limit: {:.1}%",
        settings.default_config.cpu_limit * 100.0
    );
    println!();
    ...
    match registry.execute_capability_with_microvm(
...
mod tests {
    ...
    fn test_microvm_settings() {
        ...
        assert!(settings
            .capability_configs
            .contains_key("ccos.network.http-fetch"));
        assert!(settings
            .capability_configs
            .contains_key("ccos.io.open-file"));
        ...
        assert_eq!(
            unknown_config.memory_limit_mb,
            settings.default_config.memory_limit_mb
        );
    }
}
