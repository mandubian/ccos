use crate::ast::{DelegationHint, Expression, FnExpr, Literal, ParamDef, Pattern, Symbol};
use crate::ccos::delegation::{CallContext, DelegationEngine, ExecTarget, StaticDelegationEngine};
use crate::runtime::{
    execution_outcome::ExecutionOutcome, security::RuntimeContext, values::Value, Evaluator,
    IrRuntime, ModuleRegistry,
};
...
fn test_delegation_hint_remote_model() {
    ...
    map.insert(
        "test-fn".to_string(),
        ExecTarget::RemoteModel("arbiter-remote".to_string()),
    );
    let delegation_engine = Arc::new(StaticDelegationEngine::new(map));
...
fn test_yield_based_control_flow() {
    ...
    assert!(matches!(
        result,
        ExecutionOutcome::Complete(Value::Float(42.0))
    ));
...
fn test_ir_runtime_delegation() {
    ...
    let registry_cap = Arc::new(tokio::sync::RwLock::new(
        crate::ccos::capabilities::registry::CapabilityRegistry::new(),
    ));
    let capability_marketplace =
        Arc::new(crate::ccos::capability_marketplace::CapabilityMarketplace::new(registry_cap));
