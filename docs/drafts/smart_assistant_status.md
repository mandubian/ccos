...
- ✅ Transform natural language goals into governed Intents
- ✅ Refine goals with clarifying questions (auto-answered by LLM)
- ✅ Decompose goals into executable steps
- ✅ **Recursively discover and synthesize missing capabilities** across multiple levels
- ✅ Build execution graphs with dependency resolution
- ✅ Register synthesized capabilities for reuse
- ✅ Visualize execution plans
- ✅ Handle incomplete capabilities interactively
...
### Phase 1: Foundational Reliability & Intelligence ✅
...
#### 4. Intelligent Web Search-Based OpenAPI Discovery ✅
- **Status**: Implemented
...
### Core Smart Assistant Features ✅

#### 8. Intent Extraction & Refinement ✅
- **Status**: Implemented
- **Location**: `ccos/examples/smart_assistant_demo.rs`
- **Details**: 
  - Natural language goal → Intent with constraints, acceptance criteria
...
#### 10. Capability Discovery Pipeline ✅
...
- **Discovery Priority Chain**:
  ...
  4. Recursive synthesis (convert missing capability → new intent)
...
### 3. Synthetic Capability Generation Pipeline 🔄
...
**Why It's Needed**: Many casual user goals require generic services that lack public APIs. For example, a goal like "Plan a trip to Paris" generates steps like "Search for restaurants in Paris", but there's no stable API for restaurant search—users must interact with websites like Google Maps, TheFork, or Yelp directly. Browser automation (`web.*` capabilities) provides a final fallback in the discovery chain when no API exists.
...
**Integration Point**: When discovery (Marketplace → MCP → OpenAPI → Recursive Synthesis) finds no API for capabilities like `restaurant.search` or `local.business.find`, the system should be able to synthesize a browser-based workflow with user consent for cookies and domain access.
...
### Passing Tests ✅
- ✅ All 25 discovery tests passing
- ✅ Intent extraction tests
- ✅ Plan parsing tests
- ✅ Cycle detection tests
- ✅ Intent transformation tests
...
- ✅ Web search integration: DuckDuckGo, Google, Bing with fallbacks
...
3. **Matching Algorithm**:
   - Keyword extraction: splits "github.issues.list" → ["github", "issues", "list"]
   - Normalization: handles "mcp.github.list_issues" → "list_issues" for comparison
...
- ✅ Handles: `github.issues.list` → `mcp.github.list_issues` (score > 0.7)
- ✅ Handles: `issues.list` → `mcp.github.list_issues` (score > 0.6)
- ✅ Rejects: `github.pulls.list` → `mcp.github.list_issues` (score < 0.4)
...
2. **Hybrid Matching Strategy**:
   - **First pass**: Description-based matching (`rationale` → `description`)
   - **Second pass**: Name-based matching (`capability_class` → `id`/`name`)
   - Returns best match with match type logging

3. **Applied to Both**:
   - ✅ `DiscoveryEngine::search_marketplace()` - now uses description-based search
   - ✅ `DiscoveryEngine::search_mcp_registry()` - now uses description-based search
...
- ✅ Matches even if capability ID is `mcp.github.list_issues` (name-based would also match, but description is more reliable)
...
4. **Live LLM Wiring** (Medium priority) ✅ COMPLETED (2025-01-03)
   ...
   - ✅ OpenRouter properly integrated
