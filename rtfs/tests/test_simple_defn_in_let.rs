// Simple test to verify defn in let works
...
#[test]
fn test_simple_defn_in_let() {
    // Start with just defn in let, no outer let
    let code = r#"(let [(defn helper [x] (+ x 1)) value 5] (helper value))"#;
