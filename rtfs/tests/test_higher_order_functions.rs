...
fn test_hof_param_function_enforced_on_apply() {
    ...
    assert!(ast_bad.is_err(), "AST should fail inner param type: {:?}", ast_bad);
    assert!(ir_bad.is_err(), "IR should fail inner param type: {:?}", ir_bad);
}
...
fn test_hof_variadic_inner_enforced() {
    ...
    // Mismatch: inner expects variadic Ints but receives strings after the fixed Int
        let code_bad = r#"(let [apply2 (fn [f] (f 1 "a"))
            ...
            (apply2 bad))"#;
    let (ast_bad, ir_bad) = eval_ast_and_ir(code_bad);
    assert!(ast_bad.is_err(), "AST should fail variadic rest type: {:?}", ast_bad);
    assert!(ir_bad.is_err(), "IR should fail variadic rest type: {:?}", ir_bad);
}
...
fn test_hof_return_function_applied_later_enforced() {
    ...
    // Calling returned fn with wrong type should be rejected by the inner fn's annotations
        let code_bad = r#"(let [make-adder (fn [n : Int] : :any (fn [m : Int] : Int (+ n m)))
            ...
            (adder "x"))"#;
    let (ast_bad, ir_bad) = eval_ast_and_ir(code_bad);
    assert!(ast_bad.is_err(), "AST should fail inner param type on returned fn: {:?}", ast_bad);
    assert!(ir_bad.is_err(), "IR should fail inner param type on returned fn: {:?}", ir_bad);
}
...
fn test_hof_function_type_annotation_not_yet_supported() {
    ...
    assert!(ast_res.is_err(), "AST should currently reject function type annotations for values: {:?}", ast_res);
    assert!(ir_res.is_err(), "IR should currently reject function type annotations for values: {:?}", ir_res);
}
