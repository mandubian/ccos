    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            RuntimeError::IndexOutOfBounds { index, length } => {
                write!(
                    f,
                    "Index out of bounds: index {} is not in range [0, {})",
                    index, length
                )
            }
            ...
            RuntimeError::AgentDiscoveryError {
                message,
                registry_uri,
            } => {
                write!(
                    f,
                    "Agent discovery error: {} (registry: {})",
                    message, registry_uri
                )
            }
            RuntimeError::AgentCommunicationError {
                message,
                agent_id,
                endpoint,
            } => {
                write!(
                    f,
                    "Agent communication error: {} (agent: {}, endpoint: {})",
                    message, agent_id, endpoint
                )
            }
            RuntimeError::AgentProfileError {
                message,
                profile_uri,
            } => match profile_uri {
            ...
            RuntimeError::TailCall { function, args } => {
                write!(
                    f,
                    "Tail call to function {:?} with args {:?}",
                    function, args
                )
            }
...
// TODO: Re-enable when IR is integrated
