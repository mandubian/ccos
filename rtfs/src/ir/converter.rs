...
/// # Type Theory
///
/// The **join** (least upper bound) of types τ₁ and τ₂ is:
///   join(τ₁, τ₂) = smallest τ such that τ₁ ≤ τ ∧ τ₂ ≤ τ
///
/// For vectors, we compute: join(τ₁, ..., τₙ) where τᵢ is the type of element i
///
/// # Algorithm
///
///   1. If all elements have the same type → that type
///   2. If elements are {Int, Float} → Number (Int | Float)
///   3. If elements have ≤ 5 different types → Union of those types
///   4. Otherwise → Any (too heterogeneous)
///
/// # Examples
///
/// ```text
