    ...
    match ir_result {
        Ok(original_ir) => {
            ...
            println!(
                ...
                (1.0 - optimized_stats.node_count as f64 / original_stats.node_count as f64)
                    * 100.0
            );
            println!(
                "      Operations: {} → {} ({:.2}x faster)",
            ...
            println!(
                ...
                (1.0 - optimized_stats.estimated_memory as f64
                    / original_stats.estimated_memory as f64)
                    * 100.0
            );
...
fn show_optimizations_applied(original: &IrNode, optimized: &IrNode) {
    ...
    if original_constants > optimized_constants {
        println!(
            "      • Constant folding: {} expressions pre-computed",
    ...
    if original_branches > optimized_branches {
        println!(
            "      • Branch elimination: {} dead branches removed",
    ...
    if original_nodes > optimized_nodes {
        println!(
            "      • Dead code elimination: {} nodes removed",
    ...
    match node {
            ...
            if let IrNode::VariableRef { name, .. } = function.as_ref() {
                if ["+", "-", "*", "/"].contains(&name.as_str()) {
                    ...
                    if all_literals {
                        return 1 + arguments
                            .iter()
                            .map(count_constant_expressions)
                            .sum::<usize>();
                    }
                }
            }
        ...
        IrNode::If {
            ...
            count_constant_expressions(condition)
                ...
                + else_branch
                    .as_ref()
                    .map_or(0, |e| count_constant_expressions(e))
        }
        IrNode::Let { bindings, body, .. } => {
            bindings
                .iter()
                .map(|b| count_constant_expressions(&b.init_expr))
                .sum::<usize>()
    ...
    match node {
        IrNode::If {
            ...
            1 + count_conditional_branches(condition)
                ...
                + else_branch
                    .as_ref()
                    .map_or(0, |e| count_conditional_branches(e))
        }
        IrNode::Apply {
            function,
            arguments,
            ...
            count_conditional_branches(function)
                + arguments
                    .iter()
                    .map(count_conditional_branches)
                    .sum::<usize>()
        }
        IrNode::Let { bindings, body, .. } => {
            bindings
                .iter()
                .map(|b| count_conditional_branches(&b.init_expr))
                .sum::<usize>()
...
fn count_nodes(node: &IrNode) -> usize {
    1 + match node {
        ...
        IrNode::Let { bindings, body, .. } => {
            bindings
                .iter()
                .map(|b| count_nodes(&b.pattern) + count_nodes(&b.init_expr))
                .sum::<usize>()
                + body.iter().map(count_nodes).sum::<usize>()
        }
        IrNode::Do { expressions, .. } => expressions.iter().map(count_nodes).sum::<usize>(),
        IrNode::Lambda { params, body, .. } => {
            params.iter().map(count_nodes).sum::<usize>()
                + body.iter().map(count_nodes).sum::<usize>()
        }
        IrNode::LogStep { values, .. } => values.iter().map(count_nodes).sum::<usize>(),
        _ => 0,
    }
}
