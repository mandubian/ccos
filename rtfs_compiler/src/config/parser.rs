...
impl AgentConfigParser {
    ...
    pub fn parse_agent_config_from_file<P: AsRef<std::path::Path>>(
        file_path: P,
    ) -> RuntimeResult<AgentConfig> {
        let content = std::fs::read_to_string(file_path).map_err(|e| {
            RuntimeError::Generic(format!("Failed to read agent config file: {}", e))
        })?;
        ...
        match expr {
            Expression::List(list_expr) => {
                ...
                Err(RuntimeError::Generic(
                    "Expected (agent.config ...) form".to_string(),
                ))
            }
            Expression::FunctionCall { callee, arguments } => {
                ...
                Err(RuntimeError::Generic(
                    "Expected agent.config function call".to_string(),
                ))
            }
            ...
            _ => Err(RuntimeError::Generic(
                "Expected agent.config list, function call, or map".to_string(),
            )),
        }
    }
        ...
        while i < elements.len() {
            if i + 1 >= elements.len() {
                return Err(RuntimeError::Generic(
                    "Odd number of elements in agent.config".to_string(),
                ));
            }
            ...
            if let Expression::Literal(Literal::Keyword(keyword)) = key_expr {
                ...
                return Err(RuntimeError::Generic(
                    "Expected keyword as config key".to_string(),
                ));
            }
    ...
    fn parse_agent_config_map_from_map(
        map_expr: &HashMap<MapKey, Expression>,
    ) -> RuntimeResult<AgentConfig> {
        let mut config = AgentConfig::default();
    ...
    fn parse_orchestrator_config(expr: &Expression) -> RuntimeResult<OrchestratorConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for orchestrator config".to_string(),
            )),
        }
    }
    ...
    fn parse_isolation_config(expr: &Expression) -> RuntimeResult<IsolationConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for isolation config".to_string(),
            )),
        }
    }
    ...
    fn parse_fs_config(expr: &Expression) -> RuntimeResult<FSConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for fs config".to_string(),
            )),
        }
    }
    ...
    fn parse_mounts_config(expr: &Expression) -> RuntimeResult<HashMap<String, MountConfig>> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for mounts config".to_string(),
            )),
        }
    }
    ...
    fn parse_mount_config(expr: &Expression) -> RuntimeResult<MountConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for mount config".to_string(),
            )),
        }
    }
    ...
    fn parse_dlp_config(expr: &Expression) -> RuntimeResult<DLPConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for dlp config".to_string(),
            )),
        }
    }
    ...
    fn parse_network_config(expr: &Expression) -> RuntimeResult<NetworkConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for network config".to_string(),
            )),
        }
    }
    ...
    fn parse_egress_config(expr: &Expression) -> RuntimeResult<EgressConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for egress config".to_string(),
            )),
        }
    }
    ...
    fn parse_microvm_config(expr: &Expression) -> RuntimeResult<MicroVMConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for microvm config".to_string(),
            )),
        }
    }
    ...
    fn parse_kernel_config(expr: &Expression) -> RuntimeResult<KernelConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for kernel config".to_string(),
            )),
        }
    }
    ...
    fn parse_rootfs_config(expr: &Expression) -> RuntimeResult<RootFSConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for rootfs config".to_string(),
            )),
        }
    }
    ...
    fn parse_resource_config(expr: &Expression) -> RuntimeResult<ResourceConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for resource config".to_string(),
            )),
        }
    }
    ...
    fn parse_device_config(expr: &Expression) -> RuntimeResult<DeviceConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for device config".to_string(),
            )),
        }
    }
    ...
    fn parse_nic_config(expr: &Expression) -> RuntimeResult<NICConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for nic config".to_string(),
            )),
        }
    }
    ...
    fn parse_vsock_config(expr: &Expression) -> RuntimeResult<VSockConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for vsock config".to_string(),
            )),
        }
    }
    ...
    fn parse_attestation_config(expr: &Expression) -> RuntimeResult<AttestationConfig> {
        match expr {
            ...
            _ => Err(RuntimeError::Generic(
                "Expected map for attestation config".to_string(),
            )),
        }
    }
        ...
        match expr {
            Expression::Literal(Literal::Integer(i)) => {
                if *i >= 0 && *i <= u32::MAX as i64 {
                    ...
                    Err(RuntimeError::Generic(
                        "Integer out of range for u32".to_string(),
                    ))
                }
            }
