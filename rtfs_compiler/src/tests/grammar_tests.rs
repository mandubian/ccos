        ...
        for (input, expected) in test_cases {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    assert_eq!(parsed.len(), 1, "Expected exactly one top-level item for '{}'", input);
                    ...
                    if let Some(TopLevel::Expression(Expression::Literal(Literal::Integer(val)))) = parsed.first() {
                        ...
                        if input.starts_with('-') {
                            assert!(*val < 0, "Negative input '{}' should produce negative value", input);
                        } else {
                            assert!(*val >= 0, "Non-negative input '{}' should produce non-negative value", input);
                        }
                    } else {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(Expression::Literal(Literal::Float(val)))) = parsed.first() {
                        ...
                        // Additional AST validation
                        assert!(val.is_finite(), "Float value should be finite for '{}'", input);
                        assert!(!val.is_nan(), "Float value should not be NaN for '{}'", input);
                        
                        if input.starts_with('-') {
                            assert!(*val <= 0.0, "Negative input '{}' should produce non-positive value", input);
                        } else if input.starts_with('+') || input.chars().next().unwrap().is_ascii_digit() {
                            assert!(*val >= 0.0, "Positive input '{}' should produce non-negative value", input);
                        }
                    } else {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(Expression::Literal(Literal::String(val)))) = parsed.first() {
                        ...
                        assert!(val.len() == expected.len(), "String length should match expected for '{}'", input);
                        
                        // Validate that escape sequences were properly processed
                        if input.contains("\\\"") {
                            assert!(val.contains('"'), "Escaped quotes should be unescaped in result for '{}'", input);
                        }
                        if input.contains("\\\\") {
                            assert!(val.contains('\\'), "Escaped backslashes should be unescaped in result for '{}'", input);
                        }
                        if input.contains("\\n") {
                            assert!(val.contains('\n'), "Escaped newlines should be unescaped in result for '{}'", input);
                        }
                        if input.contains("\\t") {
                            assert!(val.contains('\t'), "Escaped tabs should be unescaped in result for '{}'", input);
                        }
                        if input.contains("\\r") {
                            assert!(val.contains('\r'), "Escaped returns should be unescaped in result for '{}'", input);
                        }
                    } else {
        ...
        for (input, expected_keyword) in test_cases {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(Expression::Literal(Literal::Keyword(keyword)))) = parsed.first() {
                        let Keyword(keyword_str) = keyword;
                        assert_eq!(keyword_str, expected_keyword, "Keyword content mismatch for input: {}", input);
                        
                        // Additional AST validation for keywords
                        assert!(!keyword_str.is_empty(), "Keyword should not be empty for '{}'", input);
                        assert!(!keyword_str.starts_with(':'), "Keyword content should not include colon prefix for '{}'", input);
                        
                        // Validate namespace structure if present
                        if keyword_str.contains('/') {
                            let parts: Vec<&str> = keyword_str.split('/').collect();
                            assert_eq!(parts.len(), 2, "Namespaced keyword should have exactly one '/' for '{}'", input);
                            assert!(!parts[0].is_empty(), "Namespace part should not be empty for '{}'", input);
                            assert!(!parts[1].is_empty(), "Name part should not be empty for '{}'", input);
                        }
                        ...
                        if keyword_str.contains(':') && keyword_str.contains('/') {
                            ...
                            if main_parts.len() == 2 && main_parts[0].contains(':') {
                                ...
                                assert_eq!(ns_parts.len(), 2, "Versioned namespace should have format 'ns:version' for '{}'", input);
                                assert!(!ns_parts[0].is_empty(), "Namespace name should not be empty for '{}'", input);
                                assert!(!ns_parts[1].is_empty(), "Version should not be empty for '{}'", input);
                            }
                        }
                    } else {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(Expression::Symbol(symbol))) = parsed.first() {
                        ...
                        assert_eq!(symbol_str, input, "Symbol content should match input for '{}'", input);
                        
                        // Additional AST validation for symbols
                        assert!(!symbol_str.is_empty(), "Symbol should not be empty for '{}'", input);
                        assert!(!symbol_str.contains(' '), "Symbol should not contain spaces for '{}'", input);
                        assert!(!symbol_str.starts_with(':'), "Symbol should not start with colon for '{}'", input);
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(Expression::Symbol(symbol))) = parsed.first() {
                        ...
                        assert!(symbol_str.contains('/'), "Namespaced symbol should contain '/' for '{}'", input);
                        let parts: Vec<&str> = symbol_str.split('/').collect();
                        assert_eq!(parts.len(), 2, "Namespaced symbol should have exactly one '/' for '{}'", input);
                        assert_eq!(parts[0], expected_ns, "Namespace part mismatch for '{}'", input);
                        assert_eq!(parts[1], expected_name, "Name part mismatch for '{}'", input);
                        assert!(!parts[0].is_empty(), "Namespace should not be empty for '{}'", input);
                        assert!(!parts[1].is_empty(), "Name should not be empty for '{}'", input);
                    } else {
    ...
    fn test_empty_collections() {
        ...
        for (input, description) in test_cases {
            ...
            match result {
                Ok(parsed) => {
                    // Validate the AST structure
                    assert_eq!(parsed.len(), 1, "Expected exactly one top-level item for {}", description);
                    ...
                    if let Some(TopLevel::Expression(expr)) = parsed.first() {
                        match expr {
                            ...
                            _ => panic!("Expected collection type for {}, got: {:?}", description, expr),
                        }
                    } else {
                    ...
                    if let Some(TopLevel::Expression(expr)) = parsed.first() {
                        match expr {
                            Expression::FunctionCall { callee, arguments } => {
                                ...
                                assert_eq!(arguments.len(), 3, "Function call should have 3 arguments");
                                for (i, arg) in arguments.iter().enumerate() {
                                    if let Expression::Literal(Literal::Integer(val)) = arg {
                                        assert_eq!(*val, (i + 1) as i64, "Argument {} should be {}", i, i + 1);
                                    } else {
                            ...
                            Expression::Vector(items) => {
                                assert!(!items.is_empty(), "Vector should have items for {}", description);
                                ...
                                for (i, item) in items.iter().enumerate() {
                                    if let Expression::Literal(Literal::Integer(val)) = item {
                                        assert_eq!(*val, (i + 1) as i64, "Integer value should match position");
                                    } else {
                                        panic!("Expected integer at position {} in vector, got: {:?}", i, item);
                                    }
                                }
                            }
                            Expression::Map(items) => {
                                assert!(!items.is_empty(), "Map should have items for {}", description);
                                assert_eq!(input, r#"{"key" "value" :other 42}"#, "Map input validation");
                                assert_eq!(items.len(), 2, "Map should have 2 entries");
                                
                                // Validate specific map entries
                                assert!(items.contains_key(&MapKey::String("key".to_string())), "Map should contain string key 'key'");
                                assert!(items.contains_key(&MapKey::Keyword(Keyword("other".to_string()))), "Map should contain keyword key :other");
                                
                                if let Some(Expression::Literal(Literal::String(val))) = items.get(&MapKey::String("key".to_string())) {
                                    assert_eq!(val, "value", "String value should be 'value'");
                                }
                                
                                if let Some(Expression::Literal(Literal::Integer(val))) = items.get(&MapKey::Keyword(Keyword("other".to_string()))) {
                                    assert_eq!(*val, 42, "Integer value should be 42");
                                }
                            }
                            _ => panic!("Expected collection type or function call for {}, got: {:?}", description, expr),
                        }
                    } else {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(Expression::Let(let_expr))) = parsed.first() {
                        ...
                        assert_eq!(let_expr.bindings.len(), expected_bindings, 
                                   "Let expression should have {} bindings for '{}'", expected_bindings, input);
                        
                        // Validate body count
                        assert_eq!(let_expr.body.len(), expected_body_exprs, 
                                   "Let expression should have {} body expressions for '{}'", expected_body_exprs, input);
                        ...
                        for (i, binding) in let_expr.bindings.iter().enumerate() {
                            ...
                            match &binding.pattern {
                                ...
                                _ => panic!("Expected symbol pattern for binding at position {} in '{}'", i, input),
                            }
                            
                            // Validate binding has a value expression
                            match binding.value.as_ref() {
                                Expression::Literal(_) | Expression::Symbol(_) | Expression::List(_) => {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(Expression::If(if_expr))) = parsed.first() {
                        ...
                        if has_else_branch {
                            assert!(if_expr.else_branch.is_some(), "If expression should have else branch for '{}'", input);
                            ...
                            assert!(if_expr.else_branch.is_none(), "If expression should not have else branch for '{}'", input);
                        }
                    } else {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(Expression::Fn(fn_expr))) = parsed.first() {
                        ...
                        assert_eq!(fn_expr.params.len(), expected_params, 
                                   "Function should have {} parameters for '{}'", expected_params, input);
                        
                        // Validate variadic parameter
                        if has_variadic {
                            assert!(fn_expr.variadic_param.is_some(), "Function should have variadic parameter for '{}'", input);
                        } else {
                            assert!(fn_expr.variadic_param.is_none(), "Function should not have variadic parameter for '{}'", input);
                        }
                        
                        // Validate return type annotation
                        if has_return_type {
                            assert!(fn_expr.return_type.is_some(), "Function should have return type annotation for '{}'", input);
                        } else {
                            assert!(fn_expr.return_type.is_none(), "Function should not have return type annotation for '{}'", input);
                        }
                        ...
                        for (i, param) in fn_expr.params.iter().enumerate() {
                            match &param.pattern {
                                ...
                                _ => panic!("Expected symbol pattern for parameter at position {} in '{}'", i, input),
                            }
                        }
                        ...
                        if let Some(variadic_param) = &fn_expr.variadic_param {
                            match &variadic_param.pattern {
                                Pattern::Symbol(Symbol(name)) => {
                                    assert!(!name.is_empty(), "Variadic parameter name should not be empty for '{}'", input);
                                }
                                _ => panic!("Expected symbol pattern for variadic parameter in '{}'", input),
                            }
                        }
                        
                        // Validate function body
                        assert!(!fn_expr.body.is_empty(), "Function body should not be empty for '{}'", input);
...
mod ast_validation {
                ...
                Ok(parsed) => {
                    ...
                    for top_level in &parsed {
                        ...
                        if let Err(validation_errors) = top_level.validate() {
                            panic!("AST validation failed for '{}': {:?}", input, validation_errors);
                        }
        ...
        for (input, expected_outer, expected_inner) in test_cases {
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(expr)) = parsed.first() {
                        validate_nested_collection_structure(expr, expected_outer, expected_inner, input);
                    } else {
                        panic!("Expected expression for nested collection '{}', got: {:?}", input, parsed);
                    }
                }
    ...
    fn test_type_annotation_validation() {
        let test_cases = vec![
            ...
            ("(defn typed-add [x:int y:int]:int (+ x y))", "function with return type"),
        ];
                    ...
                    if let Some(TopLevel::Expression(expr)) = parsed.first() {
                        match expr {
                            Expression::Def(def_expr) => {
                                assert!(!def_expr.symbol.0.is_empty(), "Def symbol should not be empty for '{}'", input);
                            ...
                            Expression::Defn(defn_expr) => {
                                assert!(!defn_expr.name.0.is_empty(), "Defn name should not be empty for '{}'", input);
                                
                                // Validate return type annotation if present
                                if description == "function with return type" {
                                    assert!(defn_expr.return_type.is_some(), "Function should have return type annotation for '{}'", input);
                                    if let Some(TypeExpr::Alias(Symbol(type_name))) = &defn_expr.return_type {
                                        assert!(!type_name.is_empty(), "Return type name should not be empty for '{}'", input);
                                    }
                                }
                                ...
                                for param in &defn_expr.params {
                                    match &param.pattern {
                                        Pattern::Symbol(Symbol(name)) => {
                                            assert!(!name.is_empty(), "Parameter name should not be empty for '{}'", input);
                                        }
                                        _ => panic!("Expected symbol pattern for parameter in '{}'", input),
                                    }
                                }
                            }
                            _ => panic!("Expected def or defn expression for '{}', got: {:?}", input, expr),
                        }
                    } else {
                        panic!("Expected expression with type annotations for '{}', got: {:?}", input, parsed);
                    }
                }
                Err(e) => panic!("Failed to parse type-annotated expression '{}': {:?}", input, e),
            }
        }
    }
    ...
    fn test_pattern_matching_validation() {
        let test_cases = vec![
            ...
            ("(let [{:keys [name age]} person] (str name age))", "map destructuring"),
            ...
            match result {
                Ok(parsed) => {
                    ...
                    if let Some(TopLevel::Expression(expr)) = parsed.first() {
                        ...
                        panic!("Expected expression with patterns for '{}', got: {:?}", input, parsed);
                    }
                }
    ...
    fn validate_ast_node_consistency(top_level: &TopLevel, input: &str) {
        match top_level {
            ...
            TopLevel::Intent(intent) => {
                assert!(!intent.name.0.is_empty(), "Intent name should not be empty for '{}'", input);
                for prop in &intent.properties {
                    assert!(!prop.key.0.is_empty(), "Property key should not be empty for '{}'", input);
                }
            }
            TopLevel::Plan(plan) => {
                assert!(!plan.name.0.is_empty(), "Plan name should not be empty for '{}'", input);
            }
            TopLevel::Action(action) => {
                assert!(!action.name.0.is_empty(), "Action name should not be empty for '{}'", input);
            }
            TopLevel::Capability(capability) => {
                assert!(!capability.name.0.is_empty(), "Capability name should not be empty for '{}'", input);
            }
            TopLevel::Resource(resource) => {
                assert!(!resource.name.0.is_empty(), "Resource name should not be empty for '{}'", input);
            }
            TopLevel::Module(module) => {
                assert!(!module.name.0.is_empty(), "Module name should not be empty for '{}'", input);
            }
        }
    }
        ...
        match expr {
            Expression::Literal(lit) => {
                match lit {
                    ...
                    Literal::Keyword(Keyword(k)) => assert!(!k.is_empty(), "Keyword should not be empty for '{}'", input),
                    _ => {} // Other literals are valid
                }
            }
            Expression::Symbol(Symbol(s)) => {
                assert!(!s.is_empty(), "Symbol should not be empty for '{}'", input);
                assert!(!s.starts_with(':'), "Symbol should not start with colon for '{}'", input);
            }
            ...
            Expression::Map(map) => {
                for (key, value) in map {
                    match key {
                        MapKey::String(s) => assert!(!s.is_empty(), "Map string key should not be empty for '{}'", input),
                        MapKey::Keyword(Keyword(k)) => assert!(!k.is_empty(), "Map keyword key should not be empty for '{}'", input),
                        MapKey::Integer(_i) => {
            ...
            Expression::Let(let_expr) => {
                assert!(!let_expr.bindings.is_empty() || !let_expr.body.is_empty(), 
                        "Let expression should have bindings or body for '{}'", input);
            ...
            Expression::Defn(defn_expr) => {
                assert!(!defn_expr.name.0.is_empty(), "Defn name should not be empty for '{}'", input);
                assert!(!defn_expr.body.is_empty(), "Defn body should not be empty for '{}'", input);
    ...
    fn validate_nested_collection_structure(expr: &Expression, expected_outer: usize, expected_inner: usize, input: &str) {
        match expr {
            Expression::Vector(items) => {
                assert_eq!(items.len(), expected_outer, "Vector should have {} items for '{}'", expected_outer, input);
                for item in items {
                    if let Expression::Vector(inner_items) = item {
                        assert_eq!(inner_items.len(), expected_inner, "Inner vector should have {} items for '{}'", expected_inner, input);
                    }
                }
            }
            Expression::List(items) => {
                assert_eq!(items.len(), expected_outer, "List should have {} items for '{}'", expected_outer, input);
            }
            Expression::Map(map) => {
                assert_eq!(map.len(), expected_outer, "Map should have {} entries for '{}'", expected_outer, input);
            }
