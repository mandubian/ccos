...
fn main() {
    ...
    if graph_index.exists() {
        ...
        let index: serde_json::Map<String, Value> = serde_json::from_str(&content).expect("Invalid graph index JSON");
    ...
    if fast_plan_index.exists() && fast_intent_index.exists() {
        if let Ok(s) = fs::read_to_string(&fast_plan_index) { if let Ok(m) = serde_json::from_str(&s) { fast_plan_map = Some(m); } }
        if let Ok(s) = fs::read_to_string(&fast_intent_index) { if let Ok(m) = serde_json::from_str(&s) { fast_intent_map = Some(m); } }
    }
    if let Some(plan_map) = &fast_plan_map {
        ...
        for (plan_id, hash) in plan_map.iter() {
            ...
            let shard_path = if hash.len() >= 4 { Some(plans_dir.join(&hash[0..2]).join(&hash[2..4]).join(format!("{}.json", hash))) } else { None };
                    ...
                    if let Ok(read) = fs::read_dir(&dir) {
                        for entry in read.flatten() {
                            ...
                            if let Some(fname) = path.file_name().and_then(|s| s.to_str()) {
                                if fname == format!("{}.json", hash) { found_path = Some(path.clone()); break; }
                            }
                        }
                    }
            ...
            if let Some(p) = found_path {
                if let Ok(plan_content) = fs::read_to_string(&p) {
                    if let Ok(plan_json) = serde_json::from_str::<Value>(&plan_content) {
                        let intent_ids = plan_json.get("intent_ids").and_then(|v| v.as_array())
                    ...
                    for (hash, rel_path) in index.iter() {
                        ...
                        if plan_path.exists() {
                            if let Ok(plan_content) = fs::read_to_string(&plan_path) {
                                if let Ok(plan_json) = serde_json::from_str::<Value>(&plan_content) {
                                    let intent_ids = plan_json.get("intent_ids").and_then(|v| v.as_array());
                                    let plan_id = plan_json.get("plan_id").and_then(|v| v.as_str()).unwrap_or("");
                                    plans.push((plan_id.to_string(), intent_ids.map(|ids| ids.iter().filter_map(|id| id.as_str()).map(|s| s.to_string()).collect::<Vec<_>>()).unwrap_or_default(), hash.clone()));
                                }
                            }
                        }
                    }
    ...
    for (plan_id, intent_ids, hash) in &plans {
        println!("  - plan_id: {} (hash: {}) intents: {:?}", plan_id, hash, intent_ids);
    }
        ...
        for (iid, hashes) in intent_map.iter() {
            ...
            if let Some(plan_map) = &fast_plan_map {
                ...
                for (pid, h) in plan_map.iter() {
                    if hashes.contains(h) { pid_list.push(pid.clone()); }
                }
    ...
    println!("\nSummary: {} intents, {} plans", intents.len(), plans.len());
}
