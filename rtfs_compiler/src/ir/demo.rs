...
use crate::ir::core::*;
use crate::ir::enhanced_optimizer::{
    EnhancedIrOptimizer, EnhancedOptimizationPipeline, OptimizationLevel,
};
...
fn test_dead_code_elimination() {
    ...
    let node = IrNode::Do {
        ...
        expressions: vec![
            ...
            IrNode::Literal {
                id: 4,
                value: Literal::String("result".to_string()),
...
fn test_optimization_levels() {
    ...
    println!(
        "   - None level: {} nodes (no optimization)",
        count_nodes(&result_none)
    );
    ...
    println!(
        "   - Aggressive level: {} nodes (maximum optimization)",
        count_nodes(&result_aggressive)
    );
}

fn test_complex_optimization() {
    let mut pipeline =
        EnhancedOptimizationPipeline::with_optimization_level(OptimizationLevel::Aggressive);
    ...
    println!(
        ...
        pipeline.stats().optimization_time_ms
    );

    match optimized {
        IrNode::Let { bindings, .. } => {
            println!(
                "   - {} bindings remain (unused eliminated)",
                bindings.len()
            );
        }
        IrNode::Literal {
...
fn count_nodes(node: &IrNode) -> usize {
    match node {
        ...
        IrNode::Do { expressions, .. } => 1 + expressions.iter().map(count_nodes).sum::<usize>(),
        IrNode::Lambda { params, body, .. } => {
            1 + params.iter().map(count_nodes).sum::<usize>()
                + body.iter().map(count_nodes).sum::<usize>()
        }
        IrNode::Param { binding, .. } => 1 + count_nodes(binding),
