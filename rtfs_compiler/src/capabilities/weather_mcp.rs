//! Weather MCP Capability Implementation
//! 
//! This module implements a Model Context Protocol (MCP) capability that provides
//! weather information using the OpenWeatherMap API. It demonstrates core MCP concepts:
//! - Tool discovery and registration
//! - JSON-RPC request/response handling
//! - Schema validation with structured inputs/outputs
//! - External API integration
//! 
//! The Weather MCP server provides tools for:
...
impl MCPTool {
    pub fn new(name: String, description: String, input_schema: Value, output_schema: Option<Value>) -> Self {
        Self {
            name,
            description,
            input_schema,
...
impl WeatherMCPCapability {
    ...
    pub async fn call_tool(&mut self, tool_name: &str, arguments: &Value, context: &RuntimeContext) -> RuntimeResult<Value> {
        ...
        match tool_name {
            ...
            _ => Err(RuntimeError::UnknownCapability(
                format!("Unknown weather tool: {}", tool_name)
            )),
        }
    }
    ...
    async fn get_current_weather(&mut self, arguments: &Value) -> RuntimeResult<Value> {
        ...
        let input: CurrentWeatherInput = serde_json::from_value(arguments.clone())
            .map_err(|e| RuntimeError::Generic(
                format!("Invalid input for get_current_weather: {}", e)
            ))?;

        let cache_key = format!("current_{}_{}", 
            input.city, 
            input.country.as_deref().unwrap_or(""));

        // Check cache first (5 minute TTL)
        if let Some(cached) = self.cache.get(&cache_key) {
            if cached.timestamp.elapsed().unwrap_or(Duration::from_secs(999)) < Duration::from_secs(300) {
                return Ok(serde_json::to_value(&cached.data)
                    .map_err(|e| RuntimeError::Generic(
                        format!("Failed to serialize cached weather data: {}", e)
                    ))?);
            }
        }
        ...
        Ok(serde_json::to_value(&weather_data)
            .map_err(|e| RuntimeError::Generic(
                format!("Failed to serialize weather data: {}", e)
            ))?)
    }

    /// Get weather forecast for a city
    async fn get_weather_forecast(&mut self, arguments: &Value) -> RuntimeResult<Value> {
        let input: WeatherForecastInput = serde_json::from_value(arguments.clone())
            .map_err(|e| RuntimeError::Generic(
                format!("Invalid input for get_weather_forecast: {}", e)
            ))?;
    ...
    async fn fetch_current_weather(&self, input: &CurrentWeatherInput) -> RuntimeResult<WeatherResponse> {
        // Simulate API call delay
        ...
        Ok(WeatherResponse {
            location: format!("{}{}", 
                input.city,
                input.country.as_ref().map(|c| format!(", {}", c)).unwrap_or_default()
            ),
            temperature,
...
impl CapabilityProvider for WeatherMCPCapability {
    ...
    fn list_capabilities(&self) -> Vec<CapabilityDescriptor> {
        self.get_mcp_tools().into_iter().map(|tool| {
            ...
            CapabilityDescriptor {
                ...
                security_requirements: SecurityRequirements {
                    ...
                    network_access: NetworkAccess::Limited(vec!["api.openweathermap.org".to_string()]),
                },
                metadata,
            }
        }).collect()
    ...
    fn execute_capability(
        ...
        let tool_name = capability_id.strip_prefix("weather_mcp.")
            .ok_or_else(|| RuntimeError::UnknownCapability(
                format!("Invalid weather MCP capability ID: {}", capability_id)
            ))?;

        // Convert RuntimeValue to serde_json::Value for MCP processing
        let arguments = serde_json::to_value(inputs)
            .map_err(|e| RuntimeError::Generic(
                format!("Failed to convert inputs to JSON: {}", e)
            ))?;
        ...
        let result = rt.block_on(async {
            ...
            capability.call_tool(tool_name, &arguments, &runtime_context).await
        })?;

        // Convert back to RuntimeValue
        serde_json::from_value(result)
            .map_err(|e| RuntimeError::Generic(
                format!("Failed to convert result to RuntimeValue: {}", e)
            ))
    }
...
mod tests {
    ...
    async fn test_weather_forecast_call() {
        ...
        let result = capability.call_tool("get_weather_forecast", &arguments, &context).await;
        assert!(result.is_ok());
