...
mod tests {
    ...
    async fn test_intent_storage() {
        ...
        graph.lifecycle.infer_edges(&mut graph.storage).await.unwrap();
    ...
    async fn test_intent_relationships() {
        ...
        graph.storage.create_edge(intent2_id.clone(), intent1_id.clone(), EdgeType::DependsOn).await.unwrap();

        // Check dependent intents  
        let dependents = graph.storage.get_dependent_intents(&intent1_id).await.unwrap();
    ...
    async fn test_health_check() {
        let graph = IntentGraph::new_async(IntentGraphConfig::default()).await.unwrap();
    ...
    async fn test_weighted_edges() {
        ...
        let edge1 = crate::ccos::intent_graph::storage::Edge::new(
            ...
            intent2_id.clone(),
            EdgeType::DependsOn,
        ).with_weight(0.8).with_metadata(metadata.clone());
        ...
        let edges = graph.storage.get_edges_for_intent(&intent1_id).await.unwrap();
        
        let edge_to_intent2 = edges.iter()
        ...
        let edge_to_intent3 = edges.iter()
        ...
        let high_weight_edges: Vec<_> = edges.iter()
    ...
    async fn test_hierarchical_relationships() {
        ...
        graph.storage.create_edge(parent_id.clone(), root_id.clone(), EdgeType::IsSubgoalOf).await.unwrap();
        graph.storage.create_edge(child_id.clone(), parent_id.clone(), EdgeType::IsSubgoalOf).await.unwrap();
        
        // Debug: Check all edges
        let all_edges = graph.storage.get_edges().await.unwrap_or_else(|_| Vec::new());
        ...
        let parent_edges = graph.storage.get_edges_for_intent(&parent_id).await.unwrap();
        println!("Parent edges: {:?}", parent_edges);
        
        // Test parent relationships - manually implement the logic
        let parents_of_child: Vec<_> = child_edges.iter()
        ...
        let parents_of_parent: Vec<_> = parent_edges.iter()
        ...
        let children_of_root: Vec<_> = all_edges.iter()
        ...
        let children_of_parent: Vec<_> = all_edges.iter()
    ...
    async fn test_relationship_queries() {
        ...
        graph.storage.create_edge(intent1_id.clone(), intent2_id.clone(), EdgeType::DependsOn).await.unwrap();
        graph.storage.create_edge(intent1_id.clone(), intent3_id.clone(), EdgeType::ConflictsWith).await.unwrap();
        graph.storage.create_edge(intent1_id.clone(), intent4_id.clone(), EdgeType::Enables).await.unwrap();
        
        // Test relationship type queries - we'll need to implement this logic manually
        let edges = graph.storage.get_edges_for_intent(&intent1_id).await.unwrap();
        
        let depends_on: Vec<_> = edges.iter()
            .filter(|e| e.edge_type == EdgeType::DependsOn && e.from == intent1_id)
            .collect();
        assert_eq!(depends_on.len(), 1);
        assert_eq!(depends_on[0].to, intent2_id);
        
        let conflicts: Vec<_> = edges.iter()
            .filter(|e| e.edge_type == EdgeType::ConflictsWith && e.from == intent1_id)
            .collect();
        assert_eq!(conflicts.len(), 1);
        assert_eq!(conflicts[0].to, intent3_id);
        
        let enables: Vec<_> = edges.iter()
    ...
    async fn test_strongly_connected_intents() {
        ...
        graph.storage.create_edge(intent1_id.clone(), intent2_id.clone(), EdgeType::RelatedTo).await.unwrap();
        graph.storage.create_edge(intent2_id.clone(), intent1_id.clone(), EdgeType::RelatedTo).await.unwrap();
        
        // Create one-way relationship to intent3
        graph.storage.create_edge(intent1_id.clone(), intent3_id.clone(), EdgeType::DependsOn).await.unwrap();
        
        // Debug output
        let intent1_edges = graph.storage.get_edges_for_intent(&intent1_id).await.unwrap();
        let intent2_edges = graph.storage.get_edges_for_intent(&intent2_id).await.unwrap();
        let intent3_edges = graph.storage.get_edges_for_intent(&intent3_id).await.unwrap();
        ...
        let connected_to_intent1: Vec<_> = intent1_edges.iter()
            .filter_map(|edge| {
                let other_id = if edge.from == intent1_id { &edge.to } else { &edge.from };
        ...
        let connected_to_intent3: Vec<_> = intent3_edges.iter()
            .filter_map(|edge| {
    ...
    async fn test_virtualization_basic() {
        ...
        let virtualized = graph.create_virtualized_view(&intent_ids, &config).await.unwrap();
    ...
    async fn test_semantic_search() {
        ...
        let results: Vec<_> = all_intents.iter()
    ...
    async fn test_virtualization_with_search() {
        ...
        let search_result = graph.search_with_virtualization("AI", &config).await.unwrap();
    ...
    async fn test_virtualization_performance_stats() {
        ...
        let stored_intents = graph.storage.list_intents(IntentFilter::default()).await.unwrap();
        ...
        println!("Intent storage access completed in {:?} for {} intents", duration, stored_intents.len());
    }

    #[tokio::test]
    async fn test_virtualization_edge_cases() {
        // Test empty graph
        let empty_graph = IntentGraph::new_async(IntentGraphConfig::default()).await.unwrap();
        let result = empty_graph.create_virtualized_view(&[], &VirtualizationConfig::default()).await;
        match result {
            Ok(virtualized) => {
        ...
        let mut single_graph = IntentGraph::new_async(IntentGraphConfig::default()).await.unwrap();
        let intent = StorableIntent::new("Single intent".to_string());
        let intent_id = intent.intent_id.clone();
        single_graph.storage.store_intent(intent).await.unwrap();
        
        let single_result = single_graph.create_virtualized_view(&[intent_id], &VirtualizationConfig::default()).await.unwrap();
    ...
    async fn test_semantic_search_edge_cases() {
        ...
        let virtualized = graph.create_virtualized_view(&all_intent_ids, &VirtualizationConfig::default()).await.unwrap();
        ...
        let limited_result = graph.create_virtualized_view(&all_intent_ids, &limited_config).await.unwrap();
        println!("Limited result: {} intents, max configured: {}", limited_result.intents.len(), limited_config.max_intents);
        ...
        assert!(limited_result.intents.len() <= 2, "Expected at most 2 intents but got {}", limited_result.intents.len());
    }
}
