...
pub async fn run_mock_stream_loop(provider: &McpStreamingProvider, stream_id: String, total: usize) -> RuntimeResult<()> {
    for i in 0..total {
        let chunk = Value::Map({
            let mut m = std::collections::HashMap::new();
            m.insert(crate::ast::MapKey::Keyword(crate::ast::Keyword("seq".into())), Value::Integer(i as i64));
            m.insert(crate::ast::MapKey::Keyword(crate::ast::Keyword("payload".into())), Value::String(format!("mock-payload-{}", i)));
            m
        });
