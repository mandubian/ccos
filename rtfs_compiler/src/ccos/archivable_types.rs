...
impl Archivable for ArchivablePlan {
    fn entity_id(&self) -> String { self.plan_id.clone() }
    fn entity_type(&self) -> &'static str { "ArchivablePlan" }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ArchivablePlanLanguage { Rtfs20 }
...
impl Archivable for StorableIntent {
    fn entity_id(&self) -> String { self.intent_id.clone() }
    fn entity_type(&self) -> &'static str { "StorableIntent" }
}
...
impl Archivable for ArchivableAction {
    fn entity_id(&self) -> String { self.action_id.clone() }
    fn entity_type(&self) -> &'static str { "ArchivableAction" }
}
...
impl From<&Action> for ArchivableAction {
    fn from(action: &Action) -> Self {
        ...
        let status = match &action.result {
            Some(res) => if res.success { "Success".to_string() } else { "Failure".to_string() },
        ...
        Self {
            ...
            metadata: action.metadata.iter().map(|(k, v)| (k.clone(), format!("{:?}", v))).collect(),
        }
    }
}
...
mod tests {
    ...
    fn test_unified_archivable_storage() {
        ...
        let plan = ArchivablePlan {
            ...
            body: ArchivablePlanBody { steps: vec![], preconditions: vec![], postconditions: vec![] },
