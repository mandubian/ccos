...
use crate::ast::{Expression, Literal, MapKey, Symbol};
...
pub fn intent_to_rtfs_function_call(intent: &Intent) -> Result<Expression, RtfsBridgeError> {
    ...
    if !intent.constraints.is_empty() {
        ...
        for (key, value) in &intent.constraints {
            constraints_map.insert(MapKey::String(key.clone()), value_to_expression(value));
        }
        keyword_args.insert(
    ...
    if !intent.preferences.is_empty() {
        ...
        for (key, value) in &intent.preferences {
            preferences_map.insert(MapKey::String(key.clone()), value_to_expression(value));
        }
        keyword_args.insert(
...
pub fn plan_to_rtfs_function_call(plan: &Plan) -> Result<Expression, RtfsBridgeError> {
    ...
    match &plan.body {
        crate::ccos::types::PlanBody::Rtfs(rtfs_code) => {
            ...
            match crate::parser::parse_expression(rtfs_code) {
                Ok(body_expr) => {
                    keyword_args.insert(MapKey::String(":body".to_string()), body_expr);
                }
                Err(_) => {
    ...
    if !plan.policies.is_empty() {
        ...
        for (key, value) in &plan.policies {
            policies_map.insert(MapKey::String(key.clone()), value_to_expression(value));
        }
        keyword_args.insert(
    ...
    if !plan.annotations.is_empty() {
        ...
        for (key, value) in &plan.annotations {
            annotations_map.insert(MapKey::String(key.clone()), value_to_expression(value));
        }
        keyword_args.insert(
...
pub fn plan_to_rtfs_map(plan: &Plan) -> Result<Expression, RtfsBridgeError> {
    ...
    match &plan.body {
        crate::ccos::types::PlanBody::Rtfs(rtfs_code) => {
            ...
            match crate::parser::parse_expression(rtfs_code) {
                Ok(body_expr) => {
                    map.insert(MapKey::String(":body".to_string()), body_expr);
                }
                Err(_) => {
                    // If parsing fails, use as string literal
                    map.insert(
...
fn value_to_expression(value: &Value) -> Expression {
    match value {
        ...
        Value::Map(m) => {
            let map = m
                .iter()
                .map(|(k, v)| (k.clone(), value_to_expression(v)))
                .collect();
            Expression::Map(map)
        }
        ...
        Value::ResourceHandle(handle) => {
            Expression::Literal(Literal::ResourceHandle(handle.clone()))
        }
        ...
        Value::FunctionPlaceholder(_) => {
            Expression::Literal(Literal::String("<function_placeholder>".to_string()))
        }
