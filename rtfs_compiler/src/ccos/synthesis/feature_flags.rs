...
impl MissingCapabilityConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.feature_flags.enabled && !self.feature_flags.runtime_detection {
            return Err("Runtime detection must be enabled when missing capability resolution is enabled".to_string());
        }
        
        if self.feature_flags.auto_resolution && !self.feature_flags.enabled {
            return Err("Auto resolution requires missing capability resolution to be enabled".to_string());
        }
        ...
        if self.security_config.require_https && self.security_config.allowed_domains.is_empty() {
            return Err("At least one allowed domain must be specified when HTTPS is required".to_string());
        }
        ...
        if self.feature_flags.llm_synthesis_enabled {
            ...
            if self.llm_synthesis_config.temperature < 0.0 || self.llm_synthesis_config.temperature > 2.0 {
