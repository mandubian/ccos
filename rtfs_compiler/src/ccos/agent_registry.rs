...
pub enum TrustTier { T0Sandbox, T1Trusted, T2Privileged }
...
#[derive(Debug, Clone, Default)]
pub struct CostModel { pub cost_per_call: f64, pub tokens_per_second: f64 }
...
#[derive(Debug, Clone)]
pub struct ScoredAgent { pub descriptor: AgentDescriptor, pub score: f64, pub rationale: String, pub skill_hits: u32 }

/// Intent draft subset used for agent matching (placeholder until full Intent object integration).
#[derive(Debug, Clone)]
pub struct IntentDraft { pub goal: String, pub constraint_keys: Vec<String> }
...
impl InMemoryAgentRegistry {
    pub fn new() -> Self { Self { agents: HashMap::new(), skill_index: HashMap::new() } }
    ...
    fn score_agent(agent: &AgentDescriptor, draft: &IntentDraft) -> (f64, String, u32) {
        ...
        for s in &agent.skills { if goal_lc.contains(&s.to_lowercase()) { skill_hits += 1; } }
        let skill_component = if agent.skills.is_empty() { 0.0 } else { skill_hits as f64 / agent.skills.len() as f64 };

        // Constraint coverage
        let mut covered = 0usize;
        for c in &draft.constraint_keys { if agent.supported_constraints.iter().any(|ac| ac == c) { covered += 1; } }
        let constraint_component = if draft.constraint_keys.is_empty() { 0.0 } else { covered as f64 / draft.constraint_keys.len() as f64 };

        // Trust weight
        let trust_weight = match agent.trust_tier { TrustTier::T0Sandbox => 0.6, TrustTier::T1Trusted => 0.85, TrustTier::T2Privileged => 1.0 };

        // Cost penalty (lower cost_per_call is better)
        let cost_penalty = (agent.cost.cost_per_call / 10.0).min(0.5); // heuristic scaling
        let base = (skill_component * 0.5 + constraint_component * 0.3 + agent.success.success_rate * 0.2) * trust_weight;
        ...
        let rationale = format!(
            ...
            match agent.trust_tier { TrustTier::T0Sandbox => "T0", TrustTier::T1Trusted => "T1", TrustTier::T2Privileged => "T2" },
            cost_penalty
        );
...
impl AgentRegistry for InMemoryAgentRegistry {
    fn register(&mut self, agent: AgentDescriptor) {
        ...
        for s in &agent.skills { self.skill_index.entry(s.clone()).or_default().insert(id.clone()); }
        self.agents.insert(id, agent);
    }

    fn list(&self) -> Vec<AgentDescriptor> { self.agents.values().cloned().collect() }

    fn find_candidates(&self, draft: &IntentDraft, max: usize) -> Vec<ScoredAgent> {
        let mut scored: Vec<ScoredAgent> = self.agents.values().map(|a| {
            let (score, rationale, skill_hits) = Self::score_agent(a, draft);
            ScoredAgent { descriptor: a.clone(), score, rationale, skill_hits }
        }).collect();
