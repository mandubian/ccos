...
mod tests {
    ...
    async fn test_mock_state_provider_cas_operations() {
        ...
        let result = provider
            .kv_cas_put(key, None, initial_value.clone())
            .await
            .unwrap();
        assert!(result);
        assert_eq!(
            provider.kv_get(key).await.unwrap(),
            Some(initial_value.clone())
        );

        // CAS with correct expected value should succeed
        let result = provider
            .kv_cas_put(key, Some(initial_value), new_value.clone())
            .await
            .unwrap();
        assert!(result);
        ...
        let result = provider
            .kv_cas_put(key, Some(Value::Integer(100)), Value::Integer(200))
            .await
            .unwrap();
        assert!(!result);
    ...
    async fn test_state_provider_registry() {
        ...
        provider
            .kv_put("test", Value::String("value".to_string()))
            .await
            .unwrap();
        assert_eq!(
            provider.kv_get("test").await.unwrap(),
        ...
        let new_provider = MockStateProvider::with_initial_state(
            [(
                ...
                Value::String("pre-value".to_string()),
            )]
            .iter()
            .cloned()
            .collect(),
        ...
        assert_eq!(
            registry.provider().kv_get("pre-existing").await.unwrap(),
