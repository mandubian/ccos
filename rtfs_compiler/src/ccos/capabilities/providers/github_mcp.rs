...
use crate::ast::{MapKey, PrimitiveType, TypeExpr};
...
impl GitHubMCPCapability {
    ...
    pub async fn close_issue(
        ...
        if response.status().is_success() {
            let issue: GitHubIssue = response.json().await.map_err(|e| {
                RuntimeError::Generic(format!("Failed to parse GitHub response: {}", e))
            })?;
    ...
    pub async fn execute_tool(&self, tool_name: &str, arguments: &Value) -> RuntimeResult<Value> {
        match tool_name {
            "close_issue" => {
                let owner = arguments["owner"].as_str().ok_or_else(|| {
                    RuntimeError::Generic("Missing 'owner' parameter".to_string())
                })?;
                ...
                let issue_number = arguments["issue_number"].as_u64().ok_or_else(|| {
                    RuntimeError::Generic("Missing or invalid 'issue_number' parameter".to_string())
                })?;
            ...
            "create_issue" => {
                ...
                let title = arguments["title"].as_str().ok_or_else(|| {
                    RuntimeError::Generic("Missing 'title' parameter".to_string())
                })?;
        ...
        let json_inputs = match inputs {
            ...
            _ => {
                return Err(RuntimeError::Generic(
                    "Expected map input for GitHub MCP capability".to_string(),
                ))
            }
        };

        // Extract tool name and arguments
        let tool_name = json_inputs["tool"]
            .as_str()
...
impl GitHubMCPCapability {
    ...
    fn runtime_value_to_json(&self, value: &RuntimeValue) -> RuntimeResult<Value> {
        match value {
            ...
            RuntimeValue::FunctionPlaceholder(_) => {
                Ok(Value::String("#<function-placeholder>".to_string()))
            }
...
mod tests {
    ...
    async fn test_value_conversion() {
        ...
        let rtfs_value = RuntimeValue::Map({
            ...
            map.insert(
                MapKey::String("number".to_string()),
