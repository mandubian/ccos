...
pub trait ValidatedCapabilityProvider: CapabilityProvider {
    fn execute_capability_validated(
        ...
        let inputs_value = if inputs.len() == 1 { inputs[0].clone() } else { Value::Vector(inputs.to_vec()) };
...
impl CapabilityDescriptor {
    pub fn validate_against_type(&self, value: &Value, type_expr: &TypeExpr) -> Result<(), String> {
        match type_expr {
            TypeExpr::Primitive(PrimitiveType::Int) => {
                match value { Value::Integer(_) => Ok(()), _ => Err(format!("Expected integer, got {:?}", value)) }
            }
            TypeExpr::Primitive(PrimitiveType::String) => {
                match value { Value::String(_) => Ok(()), _ => Err(format!("Expected string, got {:?}", value)) }
            }
            TypeExpr::Intersection(types) => {
                for constraint_type in types { self.validate_against_type(value, constraint_type)?; }
                Ok(())
            }
            TypeExpr::Union(types) => {
                for union_type in types { if self.validate_against_type(value, union_type).is_ok() { return Ok(()); } }
    ...
    pub fn validate_inputs(&self, inputs: &[Value]) -> Result<(), String> {
        if let TypeExpr::Function { param_types, .. } = &self.capability_type {
            if inputs.len() != param_types.len() { return Err(format!("Expected {} parameters, got {}", param_types.len(), inputs.len())); }
            ...
            Ok(())
        } else { Err("Capability type is not a function".to_string()) }
    }
    pub fn validate_output(&self, output: &Value) -> Result<(), String> {
        if let TypeExpr::Function { return_type, .. } = &self.capability_type {
            self.validate_against_type(output, return_type).map_err(|e| format!("Return value: {}", e))
        } else { Err("Capability type is not a function".to_string()) }
    }
}
