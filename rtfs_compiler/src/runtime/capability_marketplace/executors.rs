    ...
    async fn execute(&self, provider: &ProviderType, inputs: &Value) -> RuntimeResult<Value> {
        if let ProviderType::MCP(mcp) = provider {
            ...
            let tool_name = if mcp.tool_name.is_empty() || mcp.tool_name == "*" {
                ...
                let response = client.post(&mcp.server_url)
                    .json(&tools_request)
                ...
                let tools_response: serde_json::Value = response.json().await
                ...
                if let Some(result) = tools_response.get("result") {
                    if let Some(tools) = result.get("tools").and_then(|t| t.as_array()) {
                        if let Some(first_tool) = tools.first().and_then(|t| t.get("name")).and_then(|n| n.as_str()) {
                            first_tool.to_string()
                        } else { return Err(RuntimeError::Generic("No MCP tools available".to_string())); }
                    } else { return Err(RuntimeError::Generic("Invalid tools response format".to_string())); }
                } else { return Err(RuntimeError::Generic("No result in MCP response".to_string())); }
            } else { mcp.tool_name.clone() };
            ...
            let tool_response: serde_json::Value = response.json().await
...
impl CapabilityExecutor for A2AExecutor {
    fn provider_type_id(&self) -> TypeId { TypeId::of::<A2ACapability>() }
    async fn execute(&self, provider: &ProviderType, inputs: &Value) -> RuntimeResult<Value> {
        if let ProviderType::A2A(a2a) = provider {
            match a2a.protocol.as_str() {
                "http" | "https" => self.execute_a2a_http(a2a, inputs).await,
                "websocket" | "ws" | "wss" => Err(RuntimeError::Generic("A2A WebSocket protocol not yet implemented".to_string())),
                "grpc" => Err(RuntimeError::Generic("A2A gRPC protocol not yet implemented".to_string())),
                _ => Err(RuntimeError::Generic(format!("Unsupported A2A protocol: {}", a2a.protocol)))
            }
        } else { Err(RuntimeError::Generic("ProviderType mismatch for A2AExecutor".to_string())) }
    }
}
...
impl A2AExecutor {
    async fn execute_a2a_http(&self, a2a: &A2ACapability, inputs: &Value) -> RuntimeResult<Value> {
        ...
        let response = client.post(&a2a.endpoint).header("Content-Type","application/json")
            .json(&payload).timeout(std::time::Duration::from_millis(a2a.timeout_ms)).send().await
            .map_err(|e| RuntimeError::Generic(format!("A2A HTTP request failed: {}", e)))?;
        if !response.status().is_success() {
            let status = response.status(); let error_body = response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
        ...
        if let Some(result) = response_json.get("result") { Self::json_to_rtfs_value(result) }
        else if let Some(error) = response_json.get("error") {
            let error_msg = error.get("message").and_then(|m| m.as_str()).unwrap_or("Unknown A2A error");
            Err(RuntimeError::Generic(format!("A2A error: {}", error_msg)))
        } else { Err(RuntimeError::Generic("Invalid A2A response format".to_string())) }
    }
    fn value_to_json(value: &Value) -> Result<serde_json::Value, RuntimeError> {
        ...
        match value {
            ...
            Value::Float(f) => Ok(JsonValue::Number(serde_json::Number::from_f64(*f).ok_or_else(|| RuntimeError::Generic("Invalid float value".to_string()))?)),
            Value::String(s) => Ok(JsonValue::String(s.clone())),
            Value::Boolean(b) => Ok(JsonValue::Bool(*b)),
            Value::Vector(vec) => Ok(JsonValue::Array(vec.iter().map(|v| Self::value_to_json(v)).collect::<Result<Vec<_>,_>>()?)),
            Value::Map(map) => {
                let mut json_map = serde_json::Map::new();
                for (key, val) in map {
                    let key_str = match key { MapKey::String(s) => s.clone(), MapKey::Keyword(k) => k.0.clone(), _ => return Err(RuntimeError::Generic("Map keys must be strings or keywords".to_string())), };
    ...
    fn json_to_rtfs_value(json: &serde_json::Value) -> RuntimeResult<Value> {
        match json {
            ...
            serde_json::Value::Array(arr) => Ok(Value::Vector(arr.iter().map(CapabilityMarketplace::json_to_rtfs_value).collect::<Result<Vec<_>,_>>()?)),
            serde_json::Value::Object(obj) => { let mut map = HashMap::new(); for (k,v) in obj { map.insert(MapKey::String(k.clone()), CapabilityMarketplace::json_to_rtfs_value(v)?); } Ok(Value::Map(map)) }
...
impl CapabilityExecutor for LocalExecutor {
    fn provider_type_id(&self) -> TypeId { TypeId::of::<LocalCapability>() }
    async fn execute(&self, provider: &ProviderType, inputs: &Value) -> RuntimeResult<Value> {
        if let ProviderType::Local(local) = provider { (local.handler)(inputs) }
        else { Err(RuntimeError::Generic("ProviderType mismatch for LocalExecutor".to_string())) }
    }
}
...
impl CapabilityExecutor for HttpExecutor {
    fn provider_type_id(&self) -> TypeId { TypeId::of::<HttpCapability>() }
    async fn execute(&self, provider: &ProviderType, inputs: &Value) -> RuntimeResult<Value> {
        if let ProviderType::Http(http) = provider {
            ...
            for (k,v) in response_headers.iter() { headers_map.insert(MapKey::String(k.to_string()), Value::String(v.to_str().unwrap_or("").to_string())); }
            response_map.insert(MapKey::String("headers".to_string()), Value::Map(headers_map));
            Ok(Value::Map(response_map))
        } else { Err(RuntimeError::Generic("ProviderType mismatch for HttpExecutor".to_string())) }
    }
}

pub enum ExecutorVariant { MCP(MCPExecutor), A2A(A2AExecutor), Local(LocalExecutor), Http(HttpExecutor) }

impl ExecutorVariant {
    pub async fn execute(&self, provider: &ProviderType, inputs: &Value) -> RuntimeResult<Value> {
        match self { ExecutorVariant::MCP(e)=>e.execute(provider,inputs).await, ExecutorVariant::A2A(e)=>e.execute(provider,inputs).await, ExecutorVariant::Local(e)=>e.execute(provider,inputs).await, ExecutorVariant::Http(e)=>e.execute(provider,inputs).await }
    }
}
