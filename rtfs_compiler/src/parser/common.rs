...
use super::Rule;
use crate::ast::{
    Keyword, Literal, MapDestructuringEntry, MapKey, MapMatchEntry, MatchPattern, Pattern, Symbol,
};
...
pub(super) fn build_literal(pair: Pair<Rule>) -> Result<Literal, PestParseError> {
    ...
    match inner_pair.as_rule() {
        ...
        Rule::integer => Ok(Literal::Integer(inner_pair.as_str().parse().map_err(
            |_| PestParseError::InvalidLiteral {
        ...
        Rule::string => {
            ...
            Ok(Literal::String(unescape(content).map_err(|_| {
                PestParseError::InvalidEscapeSequence {
        ...
        Rule::boolean => Ok(Literal::Boolean(inner_pair.as_str().parse().map_err(
            |_| PestParseError::InvalidLiteral {
                message: format!("Invalid boolean: {}", inner_pair.as_str()),
                span: Some(inner_span.clone()),
            },
        )?)),
...
pub(super) fn build_map_key(pair: Pair<Rule>) -> Result<MapKey, PestParseError> {
    ...
    match inner_pair.as_rule() {
        ...
        Rule::string => {
            let raw_str = inner_pair.as_str();
            let content = &raw_str[1..raw_str.len() - 1];
            Ok(MapKey::String(unescape(content).map_err(|_| {
                PestParseError::InvalidEscapeSequence {
                    sequence: content.to_string(), // Use the content that failed to unescape
                    span: Some(inner_span.clone()),
                }
        ...
        Rule::integer => Ok(MapKey::Integer(inner_pair.as_str().parse().map_err(
            |_| PestParseError::InvalidLiteral {
                message: format!("Invalid integer map key: {}", inner_pair.as_str()),
                span: Some(inner_span.clone()),
            },
        )?)),
        rule => Err(PestParseError::UnexpectedRule {
    ...
    while let Some(current_pair) = inner.next() {
        ...
        match current_pair.as_rule() {
            Rule::map_destructuring_entry => {
                ...
                let first_token_in_entry =
                    entry_inner
                        .peek()
                        .ok_or_else(|| PestParseError::MissingToken {
                            token: "first token in map_destructuring_entry".to_string(),
                ...
                if first_token_in_entry.as_rule() == Rule::keys_entry {
                ...
                } else if first_token_in_entry.as_rule() == Rule::symbol_key_binding {
                    ...
                    let key_token_pair =
                        entry_inner
                            .next()
                            .ok_or_else(|| PestParseError::MissingToken {
                                token: "map_key in map_destructuring_entry".to_string(),
                                span: Some(current_entry_span.clone()),
                            })?;
            ...
            Rule::map_rest_binding => {
                ...
                let rest_sym_pair =
                    rest_inner
                        .next()
                        .ok_or_else(|| PestParseError::MissingToken {
                            token: "symbol in map_rest_binding".to_string(),
                            span: Some(current_entry_span.clone()),
                        })?;
            ...
            Rule::map_as_binding => {
                ...
                let as_sym_pair = as_inner
                    .next()
                    .ok_or_else(|| PestParseError::MissingToken {
                        token: "symbol in map_as_binding".to_string(),
                        span: Some(current_entry_span.clone()),
                    })?;
            ...
            rule => {
                return Err(PestParseError::UnexpectedRule {
                    expected: "map destructuring entry, map_rest_binding, or map_as_binding"
                        .to_string(),
                    found: format!("{:?}", rule),
        ...
        match current_pair.as_rule() {
            ...
            Rule::vector_rest_binding => {
                ...
                let rest_sym_pair =
                    rest_inner
                        .next()
                        .ok_or_else(|| PestParseError::MissingToken {
                            token: "symbol in vector_rest_binding".to_string(),
            ...
            Rule::vector_as_binding => {
                ...
                let as_sym_pair = as_inner
                    .next()
                    .ok_or_else(|| PestParseError::MissingToken {
                        token: "symbol in vector_as_binding".to_string(),
                        span: Some(current_element_span.clone()),
                    })?;
                let as_sym_span = pair_to_source_span(&as_sym_pair);
                if as_sym_pair.as_rule() != Rule::symbol {
            ...
            rule => {
                return Err(PestParseError::UnexpectedRule {
                    expected: "binding_pattern, vector_rest_binding, or vector_as_binding"
                        .to_string(),
                    found: format!("{:?}", rule),
                    rule_text: current_pair.as_str().to_string(),
                    span: Some(current_element_span.clone()),
        ...
        Rule::vector_match_pattern => {
            ...
            while let Some(p) = inner.peek() {
                match p.as_rule() {
                    ...
                    Rule::AMPERSAND => {
                        // Assuming AMPERSAND is a rule for '&'
                        inner.next(); // Consume '&'
                        let sym_pair =
