...
impl ReentrantOrchestrator {
    ...
    pub async fn execute_with_reentrance(
        ...
        loop {
            if execution_depth >= MAX_DEPTH {
                return Err(RuntimeError::Generic("Maximum execution depth reached".to_string()));
            }
    ...
    async fn handle_effect_request(
        ...
        self.capability_marketplace.execute_capability(&effect_request.capability_id, &args_value).await
    }
}
...
pub async fn demo_reentrant_execution() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let causal_chain = Arc::new(std::sync::Mutex::new(rtfs_compiler::ccos::causal_chain::CausalChain::new()?));
    ...
    let result = orchestrator.execute_with_reentrance(&mut evaluator, &expr, &context).await?;
