...
async fn run_demo(args: Args) -> Result<(), Box<dyn Error>> {
    ...
    let mut questions = generate_clarifying_questions(
        &delegating,
        &goal,
        &intent,
        args.debug_prompts,
    ...
    let mut plan = Plan::new_rtfs(
        "(do ;; plan body will be generated by CCOS\n)".to_string(),
        vec![],
    );
    plan.metadata
...
async fn generate_clarifying_questions(
    ...
    if questions.is_empty() {
        Err(RuntimeError::Generic(
            "No clarifying questions parsed from response".to_string(),
        )
        .into())
    } else {
        Ok(questions)
...
fn parse_clarifying_response(response: &str) -> Result<Value, RuntimeError> {
    match parse_expression(response) {
...
mod tests {
    ...
    fn parses_json_plan_steps() {
        ...
        assert_eq!(step.required_inputs, vec![
            "origin".to_string(),
...
fn parse_plan_steps_response(response: &str) -> Result<Value, RuntimeError> {
    ...
    match parse_expression(&sanitized) {
        Ok(expr) => Ok(expression_to_value(&expr)),
