...
async fn main() -> Result<(), Box<dyn Error>> {
  ...
  (step \"display\"
    (call :ccos.echo (get :sum))))".to_string();

  let plan = rtfs_compiler::ccos::types::Plan::new_rtfs(rtfs_body, vec!["intent-manual-1".to_string()]);

  // Persist a minimal StorableIntent so validate_and_execute_plan can look it up
  let mut st = rtfs_compiler::ccos::types::StorableIntent::new("Manual intent for test".to_string());
