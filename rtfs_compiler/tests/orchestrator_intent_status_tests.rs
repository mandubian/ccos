...
fn test_status_transition_success() {
    ...
    let result = futures::executor::block_on(async { orchestrator.execute_plan(&plan, &ctx).await }).unwrap();
    ...
    assert!(found, "Expected an IntentStatusChanged action in causal chain for success case");
}
...
fn test_status_transition_failure() {
    ...
    assert!(found, "Expected an IntentStatusChanged action in causal chain for failure case");
}
