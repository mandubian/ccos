...
async fn test_capability_marketplace_enhanced_isolation() {
    use rtfs_compiler::ccos::capability_marketplace::{CapabilityMarketplace, NamespacePolicy};
...
async fn test_capability_marketplace_time_constraints() {
    use rtfs_compiler::ccos::capability_marketplace::{CapabilityMarketplace, TimeConstraints};
...
fn observability_prometheus_render_smoke() {
    ...
    let causal_chain = Arc::new(Mutex::new(
        rtfs_compiler::ccos::causal_chain::CausalChain::new().expect("chain"),
    ));
