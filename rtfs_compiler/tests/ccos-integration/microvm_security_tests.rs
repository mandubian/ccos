...
fn test_provider_initialization_security() {
    ...
    assert!(
        result.is_err(),
        "Uninitialized provider should fail execution"
    );
...
fn test_capability_execution_security() {
    ...
    assert!(
        result.is_ok(),
        "Valid capability should execute successfully"
    );
    ...
    assert!(
        result.is_ok(),
        "Different capability should execute successfully"
    );
    ...
    assert!(
        result.is_ok(),
        "Execution without capability ID should still work"
    );
...
fn test_provider_lifecycle_security() {
    ...
    for i in 0..3 {
        ...
        assert!(
            result.is_ok(),
            "Execution should succeed after initialization"
        );
...
fn test_error_handling_security() {
    ...
    assert!(
        result.is_ok(),
        "Initialized provider should not fail with initialization error"
    );
...
fn test_capability_access_control_implementation() {
    ...
    if result.is_err() {
        ...
        println!("  🔒 Error: {}", error);
        assert!(
            error.to_string().contains("Security violation"),
            "Should be a security violation error"
        );
    } else {
        panic!(
            "❌ SECURITY FAILURE: Unauthorized capability execution succeeded (should have failed)"
        );
    }
    ...
    if result.is_ok() {
        ...
        panic!(
            "❌ FAILURE: Authorized capability execution failed: {:?}",
            result.unwrap_err()
        );
    }
...
fn test_security_across_all_providers() {
    ...
    test_provider_security("process", || {
        providers::process::ProcessMicroVMProvider::new()
    });
    ...
    P: MicroVMProvider,
{
    ...
    if result.is_err() {
        let error = result.unwrap_err();
        println!(
            "    ✅ {} provider: Unauthorized execution blocked",
            provider_name
        );
        assert!(
            error.to_string().contains("Security violation"),
            "{} provider should return security violation error",
            provider_name
        );
    } else {
        panic!(
            "❌ {} provider: Unauthorized execution succeeded (should have failed)",
            provider_name
        );
    }
    ...
    if result.is_ok() {
        println!(
            "    ✅ {} provider: Authorized execution succeeded",
            provider_name
        );
    } else {
        println!(
            "    ⚠️  {} provider: Authorized execution failed (may be expected for some providers)",
            provider_name
        );
    }
