...
async fn test_weather_mcp_integration() {
    ...
    assert!(
        result.is_ok(),
        "Failed to call current weather tool: {:?}",
        result.err()
    );
    ...
    let forecast_result = capability
        .call_tool("get_weather_forecast", &forecast_args, &context)
        .await;
    assert!(
        forecast_result.is_ok(),
        "Failed to call forecast tool: {:?}",
        forecast_result.err()
    );
...
async fn test_weather_mcp_security_validation() {
    ...
    let mut capability = WeatherMCPCapability::new();
    let result = capability
        .call_tool("get_current_weather", &arguments, &pure_context)
        .await;

    // Should fail due to insufficient permissions
    assert!(
        result.is_err(),
        "Expected security violation, but call succeeded"
    );
