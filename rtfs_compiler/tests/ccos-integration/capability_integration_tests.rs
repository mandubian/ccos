...
async fn test_simple_capability_registration() {
    ...
    assert!(result.is_ok(), "Failed to register capability: {:?}", result.err());
    ...
    assert!(capability_ids.contains(&"double_number".to_string()), "Capability not found in registry");
}
...
async fn test_capability_execution_with_validation() {
    ...
    assert!(
        ...
        "Error message should contain type validation info: {}", error_msg
    );
}
...
async fn test_refined_type_with_predicate() {
    ...
    assert!(result.is_err());
    let error_msg = result.unwrap_err().to_string();
    assert!(error_msg.contains("Type validation failed") || error_msg.contains("length") || error_msg.contains("predicate"));
}
...
async fn test_map_type_validation() {
    ...
    let handler = Arc::new(|inputs: &Value| -> RuntimeResult<Value> {
        if let Value::Map(map) = inputs {
            let name = map.get(&rtfs_compiler::ast::MapKey::Keyword(Keyword::new("name")))
                .and_then(|v| match v {
                    Value::String(s) => Some(s.clone()),
                    _ => None,
                })
            ...
            let count = map.get(&rtfs_compiler::ast::MapKey::Keyword(Keyword::new("count")))
                .and_then(|v| match v {
    ...
    input_map.insert(rtfs_compiler::ast::MapKey::Keyword(Keyword::new("name")), Value::String("apple".to_string()));
