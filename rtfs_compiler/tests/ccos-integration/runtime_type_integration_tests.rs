...
use rtfs_compiler::ast::{PrimitiveType, TypeExpr};
...
use rtfs_compiler::runtime::type_validator::{TypeCheckingConfig, ValidationLevel};
...
mod hybrid_runtime_integration_tests {
    ...
    fn test_map_operations() {
        ...
        assert!(result.is_ok());
        match result.unwrap() {
            rtfs_compiler::runtime::execution_outcome::ExecutionOutcome::Complete(Value::Map(
                map,
            )) => {
                assert_eq!(map.len(), 2);
            }
