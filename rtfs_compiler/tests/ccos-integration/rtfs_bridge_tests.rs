use rtfs_compiler::ast::{Expression, Literal, MapKey, Symbol};
use rtfs_compiler::ccos::rtfs_bridge::{
    extract_intent_from_rtfs, extract_plan_from_rtfs, intent_to_rtfs_function_call,
    intent_to_rtfs_map, plan_to_rtfs_function_call, plan_to_rtfs_map, validate_intent,
    validate_plan, validate_plan_intent_compatibility,
};
use rtfs_compiler::ccos::types::{Intent, IntentStatus, Plan, PlanBody, PlanLanguage, PlanStatus};
        ...
        arguments: vec![
            ...
            Expression::Map(HashMap::from([
                (
                    ...
                    Expression::Literal(Literal::String("Test goal".to_string())),
                ),
                (
                    MapKey::String(":constraints".to_string()),
                    Expression::Map(HashMap::from([(
                        MapKey::String("max_cost".to_string()),
                ...
                (
                    ...
                    Expression::Map(HashMap::from([(
                        ...
                        Expression::Literal(Literal::String("high".to_string())),
                    )])),
                ),
            ])),
        ],
...
fn test_plan_function_call_extraction() {
        ...
        arguments: vec![
            ...
            Expression::Map(HashMap::from([
                (
                    ...
                    Expression::Literal(Literal::String("(do (println \"hello\"))".to_string())),
                ),
                (
                ...
                (
                    ...
                    Expression::Map(HashMap::from([(
                        MapKey::String("type".to_string()),
                        Expression::Literal(Literal::String("object".to_string())),
                    )])),
                ),
                (
                ...
                (
                    ...
                    Expression::Map(HashMap::from([
                        ...
                        (
                            MapKey::String("timeout".to_string()),
                ...
                (
                    ...
                    Expression::Map(HashMap::from([
                        (
                            MapKey::String("prompt_id".to_string()),
                            Expression::Literal(Literal::String("prompt-123".to_string())),
                        ),
                        (
                            MapKey::String("version".to_string()),
                            Expression::Literal(Literal::String("1.0".to_string())),
                        ),
                    ])),
                ),
            ])),
    ...
    assert_eq!(
        plan.annotations.get("prompt_id"),
        Some(&Value::String("prompt-123".to_string()))
    );
    assert_eq!(
        plan.annotations.get("version"),
...
fn test_intent_to_rtfs_conversion() {
    ...
    let intent = Intent {
        ...
        preferences: HashMap::from([("priority".to_string(), Value::String("high".to_string()))]),
    ...
    let function_call =
        intent_to_rtfs_function_call(&intent).expect("Failed to convert to function call");
    ...
    if let Expression::Map(map) = map_expr {
        assert_eq!(
            map.get(&MapKey::String(":type".to_string())),
            Some(&Expression::Literal(Literal::String("intent".to_string())))
        );
        assert_eq!(
        ...
        assert_eq!(
            map.get(&MapKey::String(":goal".to_string())),
            Some(&Expression::Literal(Literal::String(
                "Test goal".to_string()
            )))
        );
...
fn test_plan_to_rtfs_conversion() {
    ...
    let plan = Plan {
        ...
        annotations: HashMap::from([
            (
                "prompt_id".to_string(),
                Value::String("prompt-123".to_string()),
            ),
    ...
    // Test conversion to function call
    let function_call =
        plan_to_rtfs_function_call(&plan).expect("Failed to convert to function call");
    ...
    if let Expression::Map(map) = map_expr {
        assert_eq!(
            map.get(&MapKey::String(":type".to_string())),
            Some(&Expression::Literal(Literal::String("plan".to_string())))
        );
        assert_eq!(
            map.get(&MapKey::String(":name".to_string())),
            Some(&Expression::Literal(Literal::String(
...
fn test_round_trip_conversion() {
    ...
    let original_intent = Intent {
        ...
        original_request: "Test request".to_string(),
        goal: "Test goal".to_string(),
        constraints: HashMap::from([("max_cost".to_string(), Value::Integer(100))]),
        preferences: HashMap::from([("priority".to_string(), Value::String("high".to_string()))]),
        success_criteria: Some(Value::String("success".to_string())),
        status: IntentStatus::Active,
        created_at: 1234567890,
    ...
    let rtfs_expr =
        intent_to_rtfs_function_call(&original_intent).expect("Failed to convert to RTFS");

    // Convert back to Intent
    let converted_intent =
        extract_intent_from_rtfs(&rtfs_expr).expect("Failed to extract from RTFS");
    ...
    assert_eq!(
        converted_intent.success_criteria,
        original_intent.success_criteria
    );
    ...
    let original_plan = Plan {
        ...
        policies: HashMap::from([("max_cost".to_string(), Value::Integer(100))]),
    ...
    assert_eq!(
        converted_plan.capabilities_required,
        original_plan.capabilities_required
    );
    assert_eq!(converted_plan.annotations, original_plan.annotations);

    // Compare body
    if let (PlanBody::Rtfs(original_code), PlanBody::Rtfs(converted_code)) =
        (&original_plan.body, &converted_plan.body)
    {
