...
fn test_step_parallel_deep_merge_policy() {
    ...
    // Initialize root context and seed complex map
    {
        ...
        cm.set(
            ...
            Value::Map(
                [
                    (
                        rtfs_compiler::ast::MapKey::String("a".into()),
                        Value::Map(vec![].into_iter().collect()),
                    ),
                    (
                        rtfs_compiler::ast::MapKey::String("list".into()),
    ...
    // Note: integration via context manager since (set-context ...) is not a real form here
    {
            ...
            Value::Map(
                [
                    (
                        ...
                        Value::Map(
                            [(
                                rtfs_compiler::ast::MapKey::String("x".into()),
                                Value::Integer(10),
                            )]
                            .into_iter()
                            .collect(),
        ...
        .unwrap();
        cm.merge_child_to_parent(&b1, ConflictResolution::Merge)
            .unwrap();
            ...
            Value::Map(
                [
                    (
                        ...
                        Value::Map(
                            [(
                                rtfs_compiler::ast::MapKey::String("y".into()),
                                Value::Integer(20),
                            )]
                            .into_iter()
                            .collect(),
        ...
        .unwrap();
        cm.merge_child_to_parent(&b2, ConflictResolution::Merge)
            .unwrap();
        cm.switch_to("root").unwrap();
    }
    ...
    match merged {
        Value::Map(m) => {
            ...
            if let Some(Value::Map(a)) = m.get(&rtfs_compiler::ast::MapKey::String("a".into())) {
                assert!(a
                    .get(&rtfs_compiler::ast::MapKey::String("x".into()))
                    .is_some());
                assert!(a
                    .get(&rtfs_compiler::ast::MapKey::String("y".into()))
                    .is_some());
            } else {
                panic!("missing nested map a");
            }

            if let Some(Value::Vector(v)) =
                m.get(&rtfs_compiler::ast::MapKey::String("list".into()))
            {
                // initial [1], then branch adds [2], then [3] => at least length 3
                assert!(v.len() >= 3);
            } else {
                panic!("missing vector list");
            }
        }
