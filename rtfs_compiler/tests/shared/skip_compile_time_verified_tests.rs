use rtfs_compiler::ast::{MapKey, PrimitiveType, TypeExpr, TypePredicate};
...
mod optimization_tests {
    ...
    async fn test_marketplace_optimized_execution() {
        ...
        let result = marketplace
            .execute_with_validation_config("math.add", &params, &fast_config)
            .await;
    ...
    async fn test_optimization_performance_characteristics() {
        ...
        let complex_type = TypeExpr::Vector(Box::new(TypeExpr::Map {
            entries: vec![],
            wildcard: None,
        }));
        ...
        println!(
            "Optimized: {:?}, Strict: {:?}",
            optimized_duration, strict_duration
        );
