use chrono::Utc;
use rtfs_compiler::ast::{Keyword, MapTypeEntry, PrimitiveType, TypeExpr};
use rtfs_compiler::ccos::capabilities::registry::CapabilityRegistry;
use rtfs_compiler::ccos::capability_marketplace::{
    types::LocalCapability, CapabilityAttestation, CapabilityManifest, CapabilityMarketplace,
    CapabilityProvenance, ProviderType,
};
...
mod issue_43_tests {
    ...
    async fn test_phase_2_schema_validation() {
                    ...
                    if let Value::Map(map) = inputs {
                        if let Some(Value::String(name)) =
                            map.get(&rtfs_compiler::ast::MapKey::Keyword(
                                rtfs_compiler::ast::Keyword("name".to_string()),
                            ))
                        {
                            let mut result = HashMap::new();
                            result.insert(
                                rtfs_compiler::ast::MapKey::Keyword(rtfs_compiler::ast::Keyword(
                                    "result".to_string(),
                                )),
        ...
        let result = marketplace
            .execute_with_validation("validated_capability", &valid_params)
            .await;
        if let Err(e) = &result {
    ...
    async fn test_phase_3_content_hashing() {
        ...
        let capability = marketplace
            .get_capability("hashed_capability")
            .await
            .unwrap();
        assert!(capability.provenance.is_some());
        assert!(!capability
    ...
    async fn test_comprehensive_schema_validation() {
        ...
        marketplace
            .register_http_capability_with_schema(
        ...
        marketplace
            .register_mcp_capability_with_schema(
        ...
        marketplace
            .register_a2a_capability_with_schema(
        ...
        marketplace
            .register_plugin_capability_with_schema(
        ...
        marketplace
            .register_remote_rtfs_capability(
