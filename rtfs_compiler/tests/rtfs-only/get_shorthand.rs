...
impl HostInterface for StubHost {
    fn notify_step_started(
        &self,
        _name: &str,
    ) -> Result<String, rtfs_compiler::runtime::error::RuntimeError> {
        Ok("step-1".to_string())
    }
    fn notify_step_completed(
        &self,
        _id: &str,
        _res: &rtfs_compiler::ccos::types::ExecutionResult,
    ) -> Result<(), rtfs_compiler::runtime::error::RuntimeError> {
        Ok(())
    }
    ...
    fn get_context_value(&self, _k: &str) -> Option<rtfs_compiler::runtime::values::Value> {
        None
    }
    fn execute_capability(
        &self,
        cap: &str,
        args: &[rtfs_compiler::runtime::values::Value],
    ) -> Result<rtfs_compiler::runtime::values::Value, rtfs_compiler::runtime::error::RuntimeError>
    {
        match cap {
            ...
            "ccos.state.kv.get" => {
                ...
                if args.len() >= 1 {
                    if let rtfs_compiler::runtime::values::Value::Map(map) = &args[0] {
                        if let Some(rtfs_compiler::runtime::values::Value::String(key)) =
                            map.get(&MapKey::Keyword(Keyword("key".to_string())))
                        {
                            if key == "k" {
                                return Ok(rtfs_compiler::runtime::values::Value::String(
                                    "v".to_string(),
                                ));
                            }
                        }
...
fn test_get_shorthand_and_builtin_get() -> Result<(), rtfs_compiler::runtime::error::RuntimeError> {
    ...
    let _ = match ev.eval_toplevel(&set_items)? {
        ...
        rtfs_compiler::runtime::execution_outcome::ExecutionOutcome::RequiresHost(_) => {
            return Err(rtfs_compiler::runtime::error::RuntimeError::Generic(
