...
fn test_secure_stdlib_basic() {
    ...
    let plus_symbol = rtfs_compiler::ast::Symbol("+".to_string());
    let plus_func = env.lookup(&plus_symbol);
    assert!(plus_func.is_some(), "Plus function should be defined in secure stdlib");
...
fn test_secure_stdlib_has_core_functions() {
    ...
    let functions_to_check = vec![
        "+", "-", "*", "/", "=", ">", "<", "and", "or", "not",
        "str", "vector", "hash-map", "count", "first", "rest",
        "int?", "string?", "nil?", "empty?", "inc", "dec", "factorial"
    ];
    
    for func_name in functions_to_check {
        let symbol = rtfs_compiler::ast::Symbol(func_name.to_string());
        let func = env.lookup(&symbol);
        assert!(func.is_some(), "Function {} should be defined in secure stdlib", func_name);
    }
